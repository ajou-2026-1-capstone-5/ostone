...
- **Fix Result**: Fixed (approved) — `shared/application/exception/` 에 `BusinessException` 추상 클래스 및 `NotFoundException`, `DuplicateException`, `UnauthorizedException`, `BadRequestException`, `InvalidCredentialsException`, `InvalidTokenException` 6개 base exception 생성. auth(6개), corpus(5개), domainpack(5개) 예외 클래스가 의미론적 base class 상속으로 변경. 테스트 통과 확인.
...
- **Fix Result**: Fixed (approved) — `GlobalExceptionHandler`를 `BusinessException` 계층 타입만 처리하도록 리팩터링. 16개 외부 BC import 제거. `PasswordResetRequiredResponse` 핸들러를 `ErrorResponse`로 통일. `@ExceptionHandler(BusinessException.class)` fallback 핸들러 추가(V-029 포함). 테스트 통과 확인.
...
- **Fix Result**: Fixed (approved) — `AppUserTest.java`, `RefreshTokenTest.java`, `AuthServiceTest.java`, `AuthControllerTest.java` 4개 파일 신규 작성. 총 31개 시나리오 (signup 중복 이메일, login 잘못된 비밀번호, 만료 토큰 refresh 등 포함). 테스트 통과 확인.
...
- **Fix Result**: Fixed (approved) — `RawDatasetUploadService.java:119` `catch (Exception e)` → `catch (RuntimeException e)` 변경. 보상 로직 내부의 `catch (Exception compensationEx)`는 정리용 핸들러로 의도적이어서 유지. 테스트 통과 확인.
...
- **Fix Result**: Fixed (approved) — `ChatSession.java`에서 `setStatus()` 제거. `activate()`, `resolve()`, `reopen()` 도메인 메서드 추가. `ConsultationService`의 switch 분기도 도메인 메서드 호출로 변경. 테스트 통과 확인.
...
- **Fix Result**: Fixed (approved) — `ConsultationService`의 3개 `orElseThrow`를 `NotFoundException("SESSION_NOT_FOUND", ...)`, valueOf 실패 catch를 `BadRequestException("UNSUPPORTED_STATUS", ...)`로 교체. `ConsultationServiceTest` 기대값 업데이트. 테스트 통과 확인.
...
- **Fix Result**: Fixed (approved) — `SecurityConfig`에 `.requestMatchers("/api/v1/consultation/**").hasRole("OPERATOR")` 규칙 추가. 테스트 통과 확인.
...
- **Fix Result**: Fixed (approved) — `JwtAuthenticationFilter.java`에서 `catch (Exception ex)` → `catch (JwtException | IllegalArgumentException ex)` 변경. `import io.jsonwebtoken.JwtException` 추가. 테스트 통과 확인.
...
- **Fix Result**: Fixed (auto) — AuthService, DatasetUploadService, ActivateDomainPackVersionUseCase 3개 클래스에 `@Transactional(readOnly = true)` 적용. 쓰기 메서드에 `@Transactional` 개별 선언. 테스트 통과 확인.
...
- **Fix Result**: Fixed (auto) — `auth/{presentation,application,domain,infrastructure}/package-info.java` 4개 파일 생성. 계층별 역할 Javadoc 포함. 테스트 통과 확인.
...
- **Fix Result**: Fixed (auto) — `corpus/{presentation,application,domain,infrastructure}/package-info.java` 4개 파일 생성. 계층별 역할 Javadoc 포함. 테스트 통과 확인.
...
- **Fix Result**: Fixed (approved) — `PasswordResetRequiredResponse.java` 삭제. `GlobalExceptionHandler`에서 `ErrorResponse`로 통일 (V-002와 함께 처리). 테스트 통과 확인.
...
- **Fix Result**: Fixed (auto) — `DatasetUploadServiceTest.java` 생성. 워크스페이스 없음, 멤버십 없음, 키 중복, 정상 업로드, 중복 turnIndex 5개 시나리오 포함. 테스트 통과 확인.
...
- **Fix Result**: Fixed (approved) — 5개 테스트 파일의 메서드명을 `should_결과_when_조건` 형식으로 일괄 변경. `ConsultingContentParserTest`(7개), `ActivateDomainPackVersionUseCaseTest`(7개), `DomainPackVersionTest`(2개), `ActivateDomainPackVersionControllerTest`(7개), `ConsultationControllerTest`(4개 success 메서드) 총 27개 메서드명 변경. 테스트 통과 확인.
...
- **Fix Result**: Fixed (approved) — `ConsultingContentParserTest`(7개 메서드), `RawDatasetUploadServiceTest`(6개 메서드)에 `// given`, `// when`, `// then` (또는 `// when & then`) 주석 블록 추가. 코드 이동 없이 주석만 삽입. 테스트 통과 확인.
...
- **Fix Result**: Fixed (approved) — `DatasetController`에서 `getUserIdFromAuthentication()` private 메서드 제거. 두 엔드포인트(`uploadDataset`, `uploadRawDataset`)의 `Authentication authentication` 파라미터를 `@AuthenticationPrincipal Long userId`로 교체. 불필요해진 3개 import 제거. 테스트 통과 확인.
...
- **Fix Result**: Fixed (auto) — ConsultationService 메서드(getActiveQueue, getMessages, sendMessage, updateSessionStatus) 및 생성자 Javadoc 블록 전체 제거. 테스트 통과 확인.
...
- **Fix Result**: Fixed (auto) — 3개 실패 시나리오 추가: `@NotBlank` 위반 400, 세션 없음 5xx(현재 동작 문서화), 유효하지 않은 상태값 5xx(현재 동작 문서화). V-006 완료 후 404/400으로 업데이트 예정. 테스트 통과 확인.
...
- **Fix Result**: Fixed (auto) — `ConsultationServiceTest.java` 생성. getMessages 세션 없음, getMessages 성공, sendMessage 성공, updateSessionStatus COMPLETED, updateSessionStatus 유효하지 않은 상태 5개 시나리오 포함. 테스트 통과 확인.
...
- **Fix Result**: Fixed (approved) — `DomainPackVersion.activate()` 내에 `this.updatedAt = now;` 추가. `DomainPackVersionTest`의 해당 주석 제거 후 `assertThat(version.getUpdatedAt()).isEqualTo(now)` 검증 추가. 테스트 통과 확인.
...
- **Fix Result**: Fixed (auto) — `JpaDomainPackVersionRepositoryTest.java`(findByIdAndWorkspaceId workspace 필터 2 시나리오), `JpaDomainPackWorkspaceMembershipRepositoryTest.java`(hasAnyRole OPERATOR/VIEWER/멤버없음 3 시나리오) 생성. H2 2.x 호환(`hibernate.hbm2ddl.create_namespaces=true`). 테스트 통과 확인.
...
- **Fix Result**: Fixed (auto) — `getWorkspaceId()`, `getUserId()`, `getMemberRole()` getter 3개 추가. 테스트 통과 확인.
...
- **Fix Result**: Fixed (auto) — `JwtAuthenticationFilter`의 role==null 분기에 `log.warn("JWT token missing role claim, userId={}", userId)` 추가. 테스트 통과 확인.
...
- **Fix Result**: Fixed (auto) — workflowruntime(3), chatdemo(3), shared(3), review(3), pipelinejob(3) 총 15개 `package-info.java`에 계층별 역할 Javadoc 추가. 테스트 통과 확인.
...
- **Fix Result**: Deferred — 이메일 발송 서비스 구현 여부 설계 결정 필요. Fix Agent 범위 외.
...
- **Fix Result**: Deferred — 프로젝트 전체 아키텍처 결정 사항. Fix Agent 범위 외.
...
- **Fix Result**: Fixed (auto) — `domainpack/{application,infrastructure,presentation}/package-info.java` 3개 파일에 계층별 역할 Javadoc 추가. 테스트 통과 확인.
...
- **Fix Result**: Fixed (approved) — V-002 수정 시 `@ExceptionHandler(BusinessException.class)` fallback 핸들러를 `GlobalExceptionHandler`에 추가. 테스트 통과 확인.
...
- **Fix Result**: Deferred — 구현 미착수 단계. 구현 시작 시 적용 예정.
...
- **Fix Result**: Fixed (auto) — `ChatSession.create()` 팩토리 메서드 Javadoc, `ChatMessageRepository` 4개 메서드 Javadoc 제거. 클래스 레벨 Javadoc 유지. 테스트 통과 확인.
