...
class FakeAsgClient:
    def __init__(self, fail: bool = False, desired_capacity: int = 0, groups: list[dict] | None = None) -> None:
        self.fail = fail
    ...
    def describe_auto_scaling_groups(self, **_kwargs) -> dict:
        if self.groups is not None:
            return {"AutoScalingGroups": self.groups}
        return {"AutoScalingGroups": [{"DesiredCapacity": self.desired_capacity}]}
...
def test_current_asg_desired_capacity_reads_autoscaling_group(monkeypatch: pytest.MonkeyPatch) -> None:
    operator = _operator()
    monkeypatch.setattr(operator, "_get_asg_client", lambda: FakeAsgClient(desired_capacity=2))

    assert operator._current_asg_desired_capacity() == 2


def test_current_asg_desired_capacity_rejects_missing_group(monkeypatch: pytest.MonkeyPatch) -> None:
    operator = _operator()
    monkeypatch.setattr(operator, "_get_asg_client", lambda: FakeAsgClient(groups=[]))

    with pytest.raises(RuntimeError, match="GPU ASG not found"):
        operator._current_asg_desired_capacity()
...
def test_run_task_includes_overrides(monkeypatch: pytest.MonkeyPatch) -> None:
    operator = _operator()
    operator.overrides = {"containerOverrides": [{"name": "ml-embedder"}]}
...
def test_execute_fails_when_gpu_instance_does_not_register(monkeypatch: pytest.MonkeyPatch) -> None:
    operator = _operator()
    monkeypatch.setattr(operator, "_current_asg_desired_capacity", lambda: 0)
...
def test_execute_runs_task_and_scales_down(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    task = {
        "taskArn": "task-1",
        "lastStatus": "STOPPED",
        "containers": [{"name": "ml-embedder", "exitCode": 0}],
    }
    monkeypatch.setattr(operator, "_current_asg_desired_capacity", lambda: 0)
...
def test_execute_restores_existing_gpu_capacity(monkeypatch: pytest.MonkeyPatch) -> None:
    operator = _operator()
    desired_values: list[int] = []
    task = {
        "taskArn": "task-1",
    ...
    monkeypatch.setattr(operator, "_current_asg_desired_capacity", lambda: 1)
    monkeypatch.setattr(operator, "_scale_asg", desired_values.append)
    monkeypatch.setattr(operator, "_wait_for_gpu_instance", lambda: "container-1")
    monkeypatch.setattr(operator, "_run_task", lambda: "task-1")
    monkeypatch.setattr(operator, "_poll_task", lambda _task_arn: task)
...
def test_assert_successful_task_rejects_nonzero_exit_code() -> None:
    ...
    task = {
        "taskArn": "arn:task",
        "lastStatus": "STOPPED",
        "containers": [{"name": "ml-embedder", "exitCode": 2, "reason": "boom"}],
    }
