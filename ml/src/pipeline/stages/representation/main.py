...
def run(upstream_manifest_path: str | None = None) -> dict[str, object]:
    ...
    semantic_embeddings = (
        customer_result.embeddings * 0.45
        + full_result.embeddings * 0.20
        + resolution_result.embeddings * 0.20
        + agent_result.embeddings * 0.15
    ).astype(np.float32, copy=False)
    semantic_embeddings = _l2norm(semantic_embeddings)
