...
describe('usePolicyDetail', () => {
  ...
  it('policyId가 null이면 enabled:false로 호출한다', () => {
    mockedUseGetPolicy.mockReturnValue({ isLoading: false, refetch: vi.fn() } as unknown as ReturnType<typeof useGetPolicy>);
  ...
  it('loading 상태를 반환한다', () => {
    mockedUseGetPolicy.mockReturnValue({ isLoading: true, isFetching: true, refetch: vi.fn() } as unknown as ReturnType<typeof useGetPolicy>);
  ...
  it('error 상태를 반환한다', () => {
    ...
    mockedUseGetPolicy.mockReturnValue({ isError: true, error: err, refetch: vi.fn() } as unknown as ReturnType<typeof useGetPolicy>);
    const { result } = renderHook(() => usePolicyDetail(1, 2, 3, 4));
  ...
  it('성공 상태를 반환한다', () => {
    ...
    mockedUseGetPolicy.mockReturnValue({ isSuccess: true, data, refetch: vi.fn() } as unknown as ReturnType<typeof useGetPolicy>);
    const { result } = renderHook(() => usePolicyDetail(1, 2, 3, 4));
