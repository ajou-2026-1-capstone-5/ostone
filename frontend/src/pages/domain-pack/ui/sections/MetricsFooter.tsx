    ...
    >
      {STATS.map((stat, i) => (
        <div key={stat.label} style={{ display: "inline-flex", gap: "4px" }}>
          <Mono style={{ fontSize: "11px", color: "var(--ink-3)" }}>{stat.label}:</Mono>
          <Mono style={{ fontSize: "11px", color: "var(--ink)" }}>{stat.value}</Mono>
          {i < STATS.length - 1 && (
            <Mono style={{ fontSize: "11px", color: "var(--ink-4)", marginLeft: "4px" }}>·</Mono>
          )}
        </div>
      ))}
    </div>
