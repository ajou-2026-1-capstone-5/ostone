...
export function VersionsTimeline() {
  return (
    <div style={{ padding: "12px" }}>
      <div className="t-eyebrow" style={{ marginBottom: "8px", display: "block" }}>
        Versions
      </div>
      {VERSIONS.map((v) => (
        <div
        ...
        >
          <Mono style={{ fontSize: "11px", color: "var(--ink)" }}>{v.version}</Mono>
          <Mono style={{ fontSize: "10px", color: "var(--ink-3)" }}>{v.status}</Mono>
