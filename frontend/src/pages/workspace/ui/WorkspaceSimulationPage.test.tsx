...
describe("WorkspaceSimulationPage — QA 수정", () => {
  const readyForReviewCandidate = {
    ...candidate,
    status: "READY_FOR_REVIEW",
    reviewSessionId: 200,
    reviewTaskId: 300,
  ...
  function mockReadyForReviewCandidate() {
    mockedSimulationApi.listImprovementCandidates.mockResolvedValue({
      content: [readyForReviewCandidate],
      page: 0,
      size: 20,
      totalElements: 1,
      totalPages: 1,
  ...
  it("replay가 PASS면 성공 토스트를 띄운다", async () => {
    ...
    fireEvent.change(await screen.findByLabelText("Replay version"), {
      target: { value: "101" },
    });
    fireEvent.click(await screen.findByRole("button", { name: "환불 검증 replay" }));
  ...
  it("기대 intent/workflow/state를 선택 가능한 옵션으로 제시한다", async () => {
    renderPage();
