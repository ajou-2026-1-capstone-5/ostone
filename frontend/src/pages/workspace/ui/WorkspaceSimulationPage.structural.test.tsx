...
describe("WorkspaceSimulationPage structural 후보 검토", () => {
  ...
  it("APPLIED 후보의 리플레이 버튼 클릭 시 toast.success를 호출한다", async () => {
    const appliedCandidate = {
      ...makeStructuralCandidate(),
      status: "APPLIED" as const,
      appliedDomainPackVersionId: 505,
    };
    mockedSimulationApi.listImprovementCandidates.mockResolvedValue(
      candidatePage([appliedCandidate]),
    );
    renderPage();

    await openCandidateTab();

    // ApprovedReplayCta의 리플레이 버튼 노출 확인
    const replayButton = await screen.findByRole("button", { name: /리플레이/ });
    expect(replayButton).toBeInTheDocument();

    // 리플레이 버튼 클릭 → handleReplayAppliedVersion 실행
