...
describe('Queue', () => {
  const items = [
    { id: 'c1', name: '김민지', channel: '카카오톡', waitMinutes: 4, preview: '카드 결제 취소 문의드립니다', topic: '카드 환불', urgent: true },
    { id: 'c2', name: '이성민', channel: '라이브챗', waitMinutes: 7, preview: '부분 환불이 가능한지 확인 부탁드립니다', topic: '부분환불', urgent: true },
    { id: 'c3', name: '박정희', channel: '카카오톡', waitMinutes: 12, preview: '환불 처리 기간이 얼마나 걸리나요?', topic: '환불 처리', urgent: true },
    { id: 'c4', name: '최수진', channel: '라이브챗', waitMinutes: 3, preview: '중복 결제된 금액 확인 요청', topic: '중복 결제' },
    { id: 'c5', name: '홍길동', channel: '카카오톡', waitMinutes: 1, preview: '포인트 환불 문의', topic: '포인트' },
  ];
  ...
  it('renders 5 items with preview text', () => {
    ...
    expect(screen.getByText((content) => content.includes('카드 결제 취소 문의드립니다'))).toBeInTheDocument();
  });
