...
export function CustomerPanel({ customer, memo, onMemoChange }: CustomerPanelProps) {
  if (!customer) {
    return (
      ...
      >
        <Mono style={{ fontSize: 12, color: 'var(--ink-3)' }}>고객을 선택하면 정보가 표시됩니다</Mono>
      </div>
    );
  }
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '8px 0' }}>
          {[
            ...
            <div key={step.label} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
              ...
              {i < arr.length - 1 && (
                <span style={{ color: step.done ? 'var(--signal)' : 'var(--ink-4)', fontSize: 10 }}>→</span>
              )}
            </div>
          ))}
        </div>
      ...
      <Section title="확인된 정보">
        {[
          ...
          <div key={item.field} style={{ padding: '4px 0' }}>
            <Mono style={{ fontSize: 10, color: 'var(--ink-3)', display: 'block', marginBottom: 2 }}>{item.field}</Mono>
