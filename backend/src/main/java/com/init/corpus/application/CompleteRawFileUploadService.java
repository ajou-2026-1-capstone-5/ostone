...
/**
 * presigned 직접 업로드 흐름의 2단계. 클라이언트가 S3로 업로드를 마친 뒤 호출한다. 멤버십/소유권/세션 일치/객체 존재를 검증하고
 * {@code dataset_raw_file}을 기록한 뒤 데이터셋을 {@code PROCESSING}으로 전이하고 ML 인제스천을 트리거한다.
 *
...
public class CompleteRawFileUploadService {
   ...
   * 트랜잭션 내에서 커밋하고, Airflow 트리거는 커밋이 성공한 뒤(afterCommit)에 수행한다. 트리거가 실패해도 이미 커밋된 DB 상태는
   * 롤백되지 않으며 데이터셋은 {@code PROCESSING}으로 유지된다(재트리거는 별도 재시도 경로).
   */
  @Transactional
  public CompleteRawFileUploadResult complete(CompleteRawFileUploadCommand command) {
            ...
            .orElseThrow(
                () ->
                    new DatasetNotFoundException(
                        "데이터셋을 찾을 수 없습니다. id=" + command.datasetId()));
    ...
    ObjectMetadata metadata =
        storagePort
            ...
            .orElseThrow(
                () ->
                    new InvalidUploadStateException(
                        "업로드된 객체를 찾을 수 없습니다. 업로드를 완료한 뒤 다시 시도해 주세요."));
    ...
    storagePort.delete(session.objectKey());
    ...
    // 트리거는 트랜잭션 커밋 이후에 수행한다. 트리거 실패가 DB 상태(PROCESSING 전이·raw file 저장)를 롤백하지 않게 한다.
    registerAfterCommitTrigger(command.workspaceId(), saved.getId(), completedKey);
  ...
  private void registerAfterCommitTrigger(Long workspaceId, Long datasetId, String objectKey) {
    if (!TransactionSynchronizationManager.isSynchronizationActive()) {
      // 트랜잭션 컨텍스트가 없으면(예: 테스트) 즉시 트리거한다.
      triggerPort.trigger(workspaceId, datasetId, objectKey);
      return;
    }
  ...
  private JsonNode parseMeta(Dataset dataset) {
    ...
    try {
      ...
      throw new InvalidUploadStateException(
          "데이터셋 메타 정보를 읽을 수 없습니다. datasetId=" + dataset.getId());
    }
  }
