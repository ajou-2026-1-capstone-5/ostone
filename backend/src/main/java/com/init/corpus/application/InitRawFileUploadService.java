...
/**
 * presigned 직접 업로드 흐름의 1단계. 워크스페이스/키 검증 후 데이터셋을 {@code UPLOADING} 상태로 만들고, 업로드 세션 메타를
 * {@code meta_json}에 기록한 뒤 presigned PUT URL을 발급한다. 실제 바이트는 클라이언트가 S3로 직접 업로드한다.
 */
@Service
public class InitRawFileUploadService {
  ...
  public InitRawFileUploadResult init(InitRawFileUploadCommand command) {
    ...
    if (!ALLOWED_CONTENT_TYPES.contains(command.contentType())) {
      throw new BadRequestException(
          "VALIDATION_ERROR",
          "ZIP 파일만 업로드할 수 있습니다. contentType=" + command.contentType());
    }
