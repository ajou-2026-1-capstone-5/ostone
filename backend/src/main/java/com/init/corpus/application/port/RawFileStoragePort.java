...
public interface RawFileStoragePort {
  ...
  /**
   * 같은 버킷 내에서 server-side copy로 객체를 복제한다. presigned 업로드 완료 시 {@code pending/} 키를 {@code completed/}로
   * 승격해 lifecycle 만료 대상에서 빼는 데 쓴다. 단일 객체 최대 4GB 상한이므로 단일 CopyObjectRequest로 충분하다.
   *
