...
class PipelineDomainPackDraftCallbackControllerTest {
  ...
  @Test
  @DisplayName("동시 업데이트 충돌이면 409를 반환한다")
  @WithMockUser
  void receiveDomainPackDraftCallback_conflict_returns409() throws Exception {
    given(domainPackUseCase.execute(any())).willThrow(new PipelineJobConflictException(11L));
