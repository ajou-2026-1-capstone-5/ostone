...
본 서비스는 실시간 고객 응대 챗봇 서비스 제공보다 챗봇이 상담 과정에서 따라야 할 정책과 처리 흐름을 자동으로 생성·관리하는 데 중점을 둔다. 이를 통해 상담 기록에 포함된 반복적인 고객 요청 의도, 필수 확인 정보, 응대 정책, 위험 조건 및 처리 절차를 구조화하고, 실제 상담 자동화에 활용 가능한 운영지식으로 구성한다.
...
- MSA 형태의 다중 서비스 분산 배포는 목표로 하지 않는다.
