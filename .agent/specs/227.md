...
> **Branch**: `spec/227`
> **Depends on**: `.agent/specs/226.md` (BE read API), `frontend/DESIGN.md`, `ebb9304` (FE 디자인 개정 main 반영)
...
```mermaid
flowchart TD
    A[URL 진입: /workspaces/:wsId/packs/:packId/versions/:versionId/workflows] --> B[workflow 목록 GET 호출]
    B --> C{응답 상태}
    C -->|로딩| D[Spinner 표시]
...
│         │    ├─ JsonTab → <pre><code>{graphJson}</code></pre>
...
export function toFlow(graph: WorkflowGraph): { nodes: Node[]; edges: Edge[] } {
  const nodes: Node[] = graph.nodes.map((n, i) => ({
    id: n.id,
    type: n.type.toLowerCase(),          // 커스텀 노드 컴포넌트 이름
    data: { label: n.label },
...
커스텀 노드 타입 ↔ `GraphNode.type` 매핑:

| GraphNode.type | 커스텀 노드 CSS/컴포넌트 | 형태 |
|----------------|--------------------------|------|
| `START`    | `StartNode`    | stadium (border-radius 50px) |
| `ACTION`   | `ActionNode`   | rectangle (border-radius 6px) |
| `DECISION` | `DecisionNode` | diamond (transform rotate) |
| `ANSWER`   | `AnswerNode`   | asymmetric rectangle (좌측 컷) |
| `HANDOFF`  | `HandoffNode`  | trapezoid |
| `TERMINAL` | `TerminalNode` | circle (border-radius 50%) |
...
export function useWorkflowList(wsId: number, packId: number, versionId: number) {
  ...
  useEffect(() => {
    ...
    workflowApi.list(wsId, packId, versionId)
      ...
      .catch((e) => { if (!cancelled) setState({ status: 'error', code: e.code, message: e.message }); });
...
`useWorkflowDetail`도 동일 discriminated-union 상태 타입을 사용한다 (별도 파일).
...
```
Route (workspaces/:wsId/packs/:packId/versions/:versionId/workflows/:workflowId?)
  │
  ▼ path params
WorkflowDraftReadPage (pages)
  ├─ useWorkflowList(wsId, packId, versionId)              ── features/workflow-draft-read/model
  └─ useWorkflowDetail(wsId, packId, versionId, workflowId) ── features/workflow-draft-read/model
        ▲
...
| `src/features/workflow-draft-read/model/parseTerminalStates.ts` | new | `terminalStatesJson` 문자열 → `string[]` 파서 (실패 시 raw fallback) |
...
- 상위 탐색 진입 경로(어디에서 이 URL로 오는지)는 이 스펙 범위 밖 (별도 백로그에서 Domain Pack 탐색 UI 구축 예정).
...
- [ ] `parseTerminalStates` 정상 / 손상 / null 각 케이스
...
export function WorkflowDraftReadPage() {
  ...
  const wfId = workflowId ? Number(workflowId) : null;
...
- `@xyflow/react`는 Graph 탭 최초 접근 시점에 `React.lazy` 또는 `dynamic import`로 코드 스플리팅하여 초기 번들 영향을 최소화한다. `GraphRenderer.tsx` 자체를 lazy 컴포넌트로 export한다.
