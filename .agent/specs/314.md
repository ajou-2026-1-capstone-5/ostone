...
| Revision Draft | 현재 운영 version 전체를 새 `DRAFT` version으로 복제한 편집 초안. 현재 API 이름은 `revision-drafts`이며 intent 보정뿐 아니라 workflow 편집 진입에도 재사용한다 |
| Draft Intent 수정 | 이미 생성된 `DRAFT` version 안의 intent 정의 필드를 수정하는 작업 |
| Draft Workflow 수정 | 이미 생성된 `DRAFT` version 안의 workflow 정의 필드와 graph_json을 수정하는 작업 |
...
- Workflow 편집도 동일한 revision draft를 사용한다. Published workflow에서 편집을 시작할 때는 version 전체를 한 번 복제하고, 복제된 DRAFT version 안의 동일 `workflowCode` row를 update한다.
- DRAFT version에서 intent 또는 workflow 편집 버튼을 누르는 경우 새 draft를 만들지 않는다. 현재 DRAFT 내부 row를 바로 수정한다.
- 새 draft activate는 기존 activate API를 사용하며, 본 스펙에서 activate API에 최신 version 검증을 추가한다.
- pack당 동시에 존재 가능한 `DRAFT` version은 하나만 허용한다.
- pack당 `DRAFT` 1개 불변식은 본 스펙의 revision/restore API뿐 아니라 기존 `CreateDomainPackDraftUseCase`, `CreateDomainPackDraftFromPipelineUseCase` 등 모든 draft 생성 경로에 동일하게 적용한다.
...
| --- | --- | --- |
| `POST` | `/api/v1/workspaces/{workspaceId}/domain-packs/{packId}/versions/{versionId}/revision-drafts` | 현재 운영 version 기준 Revision Draft 생성 |
...
응답의 `sourceType`, `baseVersionId`, `baseVersionNo`, `reason`은 저장된 `summaryJson.draftSource`에서 읽어 평탄화해 내려준다.
...
`404 DOMAIN_PACK_CURRENT_VERSION_NOT_FOUND`

```json
{
  "code": "DOMAIN_PACK_CURRENT_VERSION_NOT_FOUND",
  "message": "현재 운영 중인 PUBLISHED version을 찾을 수 없습니다."
}
```
...
Workflow 편집도 같은 revision draft lifecycle을 따른다.

1. 운영자가 PUBLISHED workflow 상세에서 “편집”을 누른다.
2. FE는 `POST /versions/{versionId}/revision-drafts`를 호출한다.
3. BE는 version 전체를 새 `DRAFT` version으로 복제하며 workflow의 `workflowCode`를 유지한다.
4. FE는 새 `draftVersionId`의 workflow 목록/상세에서 원본과 같은 `workflowCode`를 가진 row를 찾아 해당 draft workflow 상세로 이동한다.
5. 운영자가 저장을 누르면 workflow PATCH API가 `DRAFT` version의 `workflowId`를 수정한다.
6. 운영자가 이미 DRAFT workflow 상세에서 “편집”을 누른 경우 2-4단계를 수행하지 않고 바로 PATCH 대상 편집 모드로 진입한다.

`revision-drafts` endpoint와 응답의 `sourceType=INTENT_REVISION` 이름은 현재 구현과의 호환을 위해 유지한다. FE는 이를 intent 전용 기능으로 해석하지 말고 Domain Pack revision draft 생성 결과로 취급한다. 추후 이름을 `REVISION` 등으로 일반화하더라도 lifecycle 의미는 바꾸지 않는다.

`PATCH` 저장 실패는 해당 intent/workflow update transaction만 rollback한다. `POST revision-drafts` 저장 실패는 clone transaction 전체를 rollback하여 draft version이 생성되지 않아야 한다. draft 생성 후 취소는 rollback이 아니라 명시적 discard/delete 작업이다.
...
pack당 `DRAFT` version은 하나만 존재할 수 있다. 이 불변식은 다음 모든 draft 생성 경로에 공통으로 적용한다.

- 신규 `CreateIntentRevisionDraftUseCase`
- 신규 `CreateRestoreDraftUseCase`
- 기존 `CreateDomainPackDraftUseCase`
- 기존 `CreateDomainPackDraftFromPipelineUseCase`

구현은 공통 draft creation guard 또는 공통 draft 생성 서비스로 모은다. 각 use case가 개별적으로 단순 `existsByDomainPackIdAndLifecycleStatus` pre-check만 수행하면 동시 요청에서 중복 `DRAFT`가 생길 수 있으므로, parent `DomainPack` row를 pessimistic write lock으로 잠근 transaction 안에서 `DRAFT` 존재 여부와 `max(versionNo)`를 다시 계산해야 한다.
...
7. 기준 version이 현재 운영 version인지 확인
   ...
   - 같은 pack에 `PUBLISHED` version이 하나도 없으면 `404 DOMAIN_PACK_CURRENT_VERSION_NOT_FOUND`
...
12. 새 draft `summaryJson.draftSource`에 `type=INTENT_REVISION`, `baseVersionId`, `baseVersionNo`, `reason` 기록
...
13. 새 draft `summaryJson.draftSource`에 restore metadata 기록
...
| `summaryJson` | 기준 summary를 복사한 뒤 `draftSource` object에 source metadata 추가 |
| `publishedAt` | `null` |
| `createdBy` | 요청 사용자 |
...
- 기준 version의 `summaryJson`을 유지한다.
- source metadata는 최상위 key를 덮어쓰지 않고 `draftSource` nested object에 기록한다.
- 기준 summary에 기존 `draftSource`가 있으면 새 draft 생성 원인을 나타내는 값으로 교체한다.

`summaryJson.draftSource` metadata 예시:
...
`WorkflowDefinition.graphJson` 내부 참조는 code 기반 참조만 허용한다. clone은 `graphJson`을 그대로 복사하며, `graphJson` 안에 intent/slot/policy/risk/workflow DB ID를 저장하거나 재매핑 대상으로 삼지 않는다. 구현 중 DB ID 기반 내부 참조가 발견되면 clone을 진행하지 말고 validation error로 실패시킨다.
...
- 현재 운영 `PUBLISHED` version이 없으면 `DOMAIN_PACK_CURRENT_VERSION_NOT_FOUND`.
...
- `summaryJson.draftSource`에 `type=INTENT_REVISION`, `baseVersionId`, `baseVersionNo`, `reason`을 기록한다.

`UpdateDraftIntentUseCaseTest`

- `DRAFT` version의 `PUBLISHED` intent 수정이면 성공한다.
- legacy `ACTIVE` intent 데이터는 migration으로 `PUBLISHED`로 정리한다. migration 이후에도 남아 있는 `ACTIVE` intent는 보정 대상으로 허용하지 않는다.
- startup/preflight data check에서 legacy `ACTIVE` intent가 남아 있으면 blocking error를 노출한다.
...
- 이미 같은 pack에 `DRAFT`가 있으면 `DOMAIN_PACK_DRAFT_ALREADY_EXISTS`.
- 동시 생성 요청은 parent pack lock 이후 재검증으로 하나만 성공하고 나머지는 `DOMAIN_PACK_DRAFT_ALREADY_EXISTS`.
- `summaryJson.draftSource`에 `type=RESTORE`, `baseVersionId`, `baseVersionNo`, `reason`을 기록한다.
...
- workflow `graphJson`은 code 기반 참조 전제로 그대로 복사한다.
...
`DomainPackDraftCreationConcurrencyTest`

- `CreateIntentRevisionDraftUseCase`, `CreateRestoreDraftUseCase`, `CreateDomainPackDraftUseCase`, `CreateDomainPackDraftFromPipelineUseCase`가 모두 공통 draft 생성 guard를 사용한다.
- 같은 pack에 대해 동시 draft 생성 transaction이 들어오면 하나만 성공하고 나머지는 `DOMAIN_PACK_DRAFT_ALREADY_EXISTS`.
- 동시성 검증은 mock unit test가 아니라 실제 DB transaction 기반 테스트로 수행한다.
...
- `pipelinejob` BC의 `ReceiveWorkflowDraftCallbackUseCase`가 `domainpack.domain`에 직접 의존하는 Cross-BC Port 리팩터링
  - 해당 구조 문제는 타당하지만 본 스펙 범위가 아니며, 별도 refactor spec인 `.agent/specs/00190.md`에서 다룬다.
...
- `pack.intent_definition.status`의 DB 기본값은 현재 `ACTIVE`이고, 코드에는 deprecated `STATUS_ACTIVE`가 남아 있다. 본 기능 구현 전 Liquibase changeset으로 legacy `ACTIVE` intent를 `PUBLISHED`로 정리하고, revision API는 `PUBLISHED`만 보정 대상으로 허용한다.
- migration 이후 startup/preflight data check에서 `pack.intent_definition.status = 'ACTIVE'` row를 검사한다. 남아 있는 row가 있으면 애플리케이션 시작을 실패시키거나 운영자가 배포를 중단할 수 있는 blocking error를 로그/헬스체크에 노출한다.
- `IntentDefinition.changeStatus()`는 기존처럼 `PUBLISHED`, `REJECTED`만 허용한다. legacy `ACTIVE`를 새 상태 전이 대상으로 다시 허용하지 않는다.
- `slot_definition`, `policy_definition`, `risk_definition`의 `status` 컬럼은 초기 DDL이 아니라 후속 Liquibase changeset으로 추가되어 있다. 본 스펙 구현에는 해당 컬럼 추가 migration을 새로 만들지 않고, clone 시 기존 `ACTIVE/INACTIVE` status를 명시적으로 복사한다.
- `WorkflowDefinition.graphJson` 내부 참조는 code 기반이어야 한다. DB ID 기반 참조를 새로 도입하지 않는다.
