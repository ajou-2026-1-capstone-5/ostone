...
- `pack.intent_definition.status`의 DB 기본값은 현재 `ACTIVE`이고, 코드에는 deprecated `STATUS_ACTIVE`가 남아 있다. 본 기능 구현 전 legacy `ACTIVE` intent는 migration으로 `PUBLISHED`로 정리하고, revision API는 `PUBLISHED`만 보정 대상으로 허용한다.
