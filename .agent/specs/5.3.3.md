...
> **참고**: 하위 클래스(`ChatController`, `LlmAssistantService`, `SpringAiChatClientConfig`)는
> 이 스펙에 따라 5.3.3 구현 단계에서 신규 생성 예정이다.
> 현재는 `workflow-runtime` 모듈 내에 존재하지 않으며, DDD 계층 구조를 따라 신규 패키지에 추가한다.

`backend/src/main/java/com/init/workflowruntime/presentation/ChatController.java`

* 기존 사용자 메시지 STOMP 처리 흐름을 유지한다.
...
`backend/src/main/java/com/init/workflowruntime/infrastructure/SpringAiChatClientConfig.java`
