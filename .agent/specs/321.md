...
- [ ] `SlotEditSheet`: Sheet open 시 GET slot 데이터 로딩, loading/error/populated 3종 상태 처리; 에러 시 Sheet 유지하고 재시도 가능한 상태로 표시 (Sheet 닫기 금지)
...
- [ ] status/fields PATCH 성공 시 `slotKeys.detail` + `slotKeys.list` 두 캐시 모두 무효화 (`useUpdateSlotStatus`, `useUpdateSlot` 모두 동일하게 적용)
- [ ] `alert()` 미사용, toast 사용
