...
컴포넌트 분리는 필수는 아니다. 단 unavailable 안내가 `ChatPanel`의 일반 empty state와 함께 커져서 책임이 흐려지면 `ConsultationPage` 근처에 작은 전용 view 컴포넌트로 분리한다.
...
- `sessionId`가 있으면 대기열 로드 완료 후 `queue.find((customer) => customer.id === sessionId)`로 접근 가능성을 확인한다.
...
- URL 세션이 queue에 존재할 때만 `consultationApi.getMessages(Number(sessionId))`를 호출한다.
...
Route /workspaces/:workspaceId/consultation/:sessionId
  ...
  -> queue contains sessionId?
       yes:
         ...
         load messages / matched workflow / websocket topic as today
       no:
