...
sequenceDiagram
    ...
    repo ->> db: SELECT pack.domain_pack_version WHERE id = ?
    db ->> repo: DomainPackVersion row
    repo ->> uc: DomainPackVersion
    uc ->> uc: packId 일치 검증
    uc ->> uc: lifecycle_status 전이 가능 여부 검증
    uc ->> uc: version.activate()
    uc ->> repo: save(version)
    repo ->> db: UPDATE pack.domain_pack_version SET lifecycle_status='PUBLISHED', published_at=NOW(), updated_at=NOW() WHERE id=?
    db ->> repo: ok
...
public class ActivateDomainPackVersionUseCase {
    ...
    public ActivateDomainPackVersionUseCase(DomainPackVersionRepository versionRepository) {
    ...
    public ActivateDomainPackVersionResult execute(ActivateDomainPackVersionCommand command) {
        ...
        version.activate(); // 전이 규칙 적용 (U-001 참조)
...
> `workspaceId` 검증(workspace 존재 확인, workspace membership 확인) 포함 여부는 **U-005** 참조.
...
class DomainPackVersionTest {
    ...
    void activate_fromNonPublished_setsPublished() {
        // given: lifecycle_status != PUBLISHED 인 version
        // when
        version.activate();
        // then
...
- [ ] 멱등성: 이미 PUBLISHED 상태 재시도 → 에러 반환 (400)
...
- `lifecycle_status` 전이 허용 범위(선처리 상태)는 확정되지 않았다. **U-001** 참조.
- Domain event 발행 여부는 확정되지 않았다. **U-004** 참조.
