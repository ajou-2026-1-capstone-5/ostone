...
classDiagram
    ...
    class WorkspaceMembershipPort {
        <<interface>>
        +existsByWorkspaceIdAndUserIdAndMemberRoleIn(Long, Long, List): boolean
    }
...
public class ActivateDomainPackVersionUseCase {
    ...
    public ActivateDomainPackVersionResult execute(ActivateDomainPackVersionCommand command) {
        ...
        // workspace 멤버십 + 역할 확인 (U-005 Confirmed): OPERATOR 또는 ADMIN만 허용
...
- `WorkspaceMembershipPort` 어댑터 구현 시 주의: corpus의 `WorkspaceMemberRef` 엔티티는 `member_role` 컬럼을 매핑하지 않는다 (`WorkspaceMemberRef.java` 확인됨). domainpack의 인프라 어댑터는 `member_role`이 포함된 별도 read model(또는 JPQL)을 사용해야 한다. corpus 어댑터를 그대로 재사용하면 role 필터링 불가.
