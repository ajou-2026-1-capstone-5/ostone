...
|--------|------|-------------|
| GET | `/api/v1/workspaces/:workspaceId/chat/sessions/current` | 현재 사용자 채팅 세션 조회 또는 생성. 기존 OPEN/ACTIVE 세션이 있으면 재사용하고, 없으면 현재 PUBLISHED domain pack version으로 OPEN 세션 생성 |
| GET | `/api/v1/consultation/sessions/:sessionId/messages` | 상담사 화면용 채팅 히스토리 조회. 사용자 채팅 히스토리는 별도 전용 API 없이 이 조회 API를 재사용한다 |
| POST | `/api/v1/consultation/sessions/:sessionId/messages` | 상담사 화면용 메시지 전송. 사용자 발신 메시지는 STOMP `/app/chat.sendMessage`를 사용한다 |

### STOMP WebSocket

| 구분 | 값 | 설명 |
|------|----|------|
| 연결 엔드포인트 | `/ws/chat` | JWT Bearer 토큰을 STOMP CONNECT 헤더에 포함 |
| Subscribe destination | `/topic/chat.{sessionId}` | 세션별 메시지 수신 |
| Publish destination | `/app/chat.sendMessage` | 사용자 메시지 전송 |
| 수신 payload | `ChatMessageResponse` (`id`, `seqNo`, `senderRole`, `messageType`, `content`, `createdAt`) | 프론트는 topic의 `sessionId`와 `senderRole`을 UI 모델로 변환 |
...
| `frontend/src/pages/user-chat/ui/UserChatPage.tsx` | new | 라우트 파라미터를 읽고 사용자 채팅 화면 구성 |
| `frontend/src/pages/user-chat/index.ts` | new | 페이지 public export |
| `frontend/src/entities/chat/api/chatApi.ts` | new | current session API wrapper |
| `frontend/src/entities/chat/model/types.ts` | new | Message, Session, ConnectionStatus 타입 정의 |
| `frontend/src/features/user-chat/ui/ChatRoom.tsx` | new | STOMP 구독, 메시지 변환, 메시지 목록, 입력 조합 |
| `frontend/src/features/user-chat/ui/MessageInput.tsx` | new | 메시지 입력, 전송 버튼 |
| `frontend/src/features/user-chat/ui/MessageList.tsx` | new | 메시지 목록과 자동 스크롤 처리 |
| `frontend/src/features/user-chat/ui/ConnectionStatus.tsx` | new | 연결 상태 표시 |
| `frontend/src/features/user-chat/index.ts` | new | feature slice public export |
| `frontend/src/entities/chat/index.ts` | new | entity public export |
| `frontend/src/shared/lib/websocket/stompClient.ts` | new | shared 레벨 STOMP WebSocket client 생성 |
| `frontend/src/shared/lib/websocket/useStomp.ts` | new | STOMP 연결, 구독, publish, 재연결 상태 관리 |
...
| shared WebSocket | STOMP client 생성과 연결 관리는 `frontend/src/shared/lib/websocket/stompClient.ts`, `frontend/src/shared/lib/websocket/useStomp.ts`에 둔다. |
