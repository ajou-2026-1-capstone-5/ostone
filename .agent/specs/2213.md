...
특정 Domain Pack Version에 속한 Risk Factor 단건을 조회하는 READ 전용 엔드포인트를 제공한다.
...
|--------|------|-------------|
| GET | `/api/v1/workspaces/{workspaceId}/domain-packs/{packId}/versions/{versionId}/risks/{riskId}` | Risk Factor 단건 조회 |
...
`JpaRiskDefinitionRepository`는 이미 `JpaRepository<RiskDefinition, Long>`과 `RiskDefinitionRepository`를 모두 extends하고 있으므로 수정 불필요. Spring Data JPA가 도메인 인터페이스에 선언된 메서드 이름을 상속하여 자동 구현한다.
...
| 단건 미존재 | 404, `body.code == "RISK_DEFINITION_NOT_FOUND"` |
| 권한 없음 | 403 |
| 인증 없음 | 401 |
| version 미존재 | 404, `body.code == "DOMAIN_PACK_VERSION_NOT_FOUND"` |
...
- `findByIdAndDomainPackVersionId`는 `RiskDefinitionRepository`(도메인 인터페이스)에만 선언한다. `JpaRiskDefinitionRepository`에 재선언하면 안 된다 — 이미 양쪽 인터페이스를 extends하고 있어 Spring Data JPA가 이름 기반으로 자동 구현한다.
