...
- `webhook_receipt`가 `FAILED` 또는 `RECEIVED` 상태면 재처리 가능하고, `PROCESSED`일 때만 duplicate 로 무시
- `pipeline_job.version` 기반 optimistic locking으로 동시 callback 경쟁을 409 충돌로 차단
- 이미 저장된 부모 intent를 참조하는 incremental `parentIntentCode` 연결 지원
- `IntentDraft`를 공용 application DTO로 분리해 callback 경로와 운영자 draft 생성 경로가 같은 intent 구조를 사용
...
sequenceDiagram
    ...
    cb ->> receiptRepo: saveAndFlush(RECEIVED) or reuse FAILED/RECEIVED receipt
...
- `persistIntents(...)`
  - 대상 version이 존재하는지, `DRAFT` 상태인지 먼저 검증
  - payload 내부 `intentCode` 중복 검증
  - 이미 존재하는 `intentCode`는 skip
  - 새 intent를 먼저 flush 해서 ID를 확보한 뒤 parent-child 연결을 반영
...
- `PROCESSED` receipt만 duplicate 로 간주하고, `FAILED` / `RECEIVED` receipt는 재처리 대상으로 본다
- 처리 상태:
  - `RECEIVED`
  - `PROCESSED`
  - `FAILED`
...
- `version`
  - JPA `@Version` 컬럼
  - 동시 callback이 같은 job 상태를 덮어쓰지 못하도록 CAS 성격의 충돌 검출에 사용
...
- Spring Security에서 아래 POST 경로만 `permitAll`
  - `/api/v1/pipeline-jobs/*/callbacks/domain-pack-drafts`
  - `/api/v1/pipeline-jobs/*/callbacks/intent-drafts`
- 실제 인증은 use case 내부에서 `X-Airflow-Webhook-Secret`와 `airflow.webhook.secret`를 상수 시간 비교로 검증한다
- 저장되는 request header JSON에서는 secret 값을 `***`로 마스킹한다
- CORS allowed header에 `X-Airflow-Webhook-Secret`를 추가했다
- 운영 기본 설정에서는 `AIRFLOW_WEBHOOK_SECRET`를 반드시 주입하고, local 프로필만 개발용 기본값을 둔다
...
- `DomainPackDraftPersistenceServiceTest`
  ...
  - 같은 요청에서 새 부모/새 자식 intent 연결
  - version not found / non-DRAFT version 거부
...
- `PipelineIntentDraftCallbackControllerTest`
  - 201 / 200 / 401 / 404 / 409(finalized/not-allowed) / 400
