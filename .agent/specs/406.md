...
| `backend/src/main/java/com/init/workflowruntime/application/exception/AiResponseInProgressException.java` | new | demo HTTP busy 상태를 `DuplicateException` 기반 409로 노출 |
...
| Application unit | demo append가 busy 시 409 예외를 던지고 저장하지 않는지 검증 | JUnit 5 + Mockito | `AiResponseInProgressException` 확인 |
