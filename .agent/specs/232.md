...
> **U-FE-232-5-B 의존성**: "파이프라인 import" 탭은 `GET .../pipeline-jobs/{jobId}/artifacts` (또는 동등한) BE 엔드포인트가 필요하다. 해당 엔드포인트 존재 여부는 현재 미확인이며 구현 전 BE 확인 필수. 미존재 시 해당 탭은 비활성("준비 중") 처리한다.
```
