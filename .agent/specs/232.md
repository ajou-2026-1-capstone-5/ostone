  ...
  return useMutation({
    ...
    onError: (error: ApiRequestError) => {
      if (error.status === 403) toast.error('접근 권한 없음');
        ...
        // 409는 caller(모달)의 per-call onError에서 처리한다. toast 없음.
      } else {
...
### CreateDraftModal — 409 인라인 에러 연결 패턴

`useCreateDraft`의 409는 hook 전역 `onError`에서 toast를 내지 않고, 모달이 `mutate` 호출 시 per-call `onError`로 직접 주입받는다.

```tsx
// features/domain-pack-draft-create/ui/CreateDraftModal.tsx (핵심 부분)
export function CreateDraftModal({ wsId, packId, onClose, onSuccess }: Props) {
  const mutation = useCreateDraft();
  const [inlineError, setInlineError] = useState<string | null>(null);

  const handleSubmit = (payload: CreateDomainPackDraftRequest) => {
    setInlineError(null);
    mutation.mutate(
      { wsId, packId, payload },
      {
        onSuccess: (data) => {
          onSuccess(data.versionId);   // 모달 닫기 + 새 버전 선택은 부모에서 처리
        },
        onError: (error: ApiRequestError) => {
          if (error.status === 409) {
            setInlineError('동일 Pack 묶기 충돌. 잠시 후 재시도하세요.');
          }
          // 403/generic는 useCreateDraft onError에서 이미 toast 처리됨
        },
      },
    );
  };
  ...
  return (
    <dialog role="dialog" aria-modal>
      {/* ... 입력 탭, textarea, 파일 업로드 ... */}
      {inlineError && <p role="alert">{inlineError}</p>}
      <button onClick={onClose}>취소</button>
      <button onClick={...} disabled={mutation.isPending}>묶기</button>
    </dialog>
  );
}
```

> - `inlineError` 상태는 `handleSubmit` 호출 시마다 초기화한다 (재시도 시 이전 에러 제거).
> - per-call `onError`는 hook 전역 `onError` **이후** 실행된다 (TanStack Query v5 동작). 409 toast가 hook에서 나오지 않으므로 중복 노출이 없다.
> - 400(서버 검증 실패)의 필드별 에러는 `error.data.errors` 배열을 파싱하여 동일 `inlineError` 혹은 `fieldErrors` 상태에 매핑한다.
