...
│         │    ├─ PolicyCountCard   ("policy · 5" → /workspaces/.../policies, BE GET 미존재 시 disabled)
│         │    ├─ RiskCountCard     ("risk · 3"   → /workspaces/.../risks, BE GET 미존재 시 disabled)
...
- 컴포넌트 카운트 카드 클릭 시 기존/예정 라우트로 navigate:
  ...
  - policy → 동일 패턴 (FE 백로그 2.2.10, BE GET 선결 필요)
  - risk → 동일 패턴 (FE 백로그 2.2.12, BE GET 선결 필요)
...
| 9 | policy/risk BE 미존재 | 카운트 0/disabled | 카드 disabled + 툴팁 "준비 중" |
