...
`graphJson`을 `@JsonRawValue`로 반환하여 HTTP 응답에서 JSON object로 직렬화된다. 직렬화 전에 `WorkflowDefinitionDetail.from()` 팩토리 메서드에서 `graphJson` 값의 JSON 파싱 유효성을 검사하며, 파싱 실패 시 `WORKFLOW_GRAPH_JSON_INVALID` 코드(500)로 응답한다.
...
> `graphJson` 필드: 응답 DTO에서 `@JsonRawValue`를 사용해 raw JSON으로 직렬화. 엔티티는 여전히 String으로 보관. `WorkflowDefinitionDetail.from()` 팩토리 메서드에서 JSON 파싱 유효성 검사 후 실패 시 `WORKFLOW_GRAPH_JSON_INVALID`(500) 예외 발생.
...
- [ ] DB에 유효하지 않은 graphJson이 있는 경우 → 500 WORKFLOW_GRAPH_JSON_INVALID 반환
...
- `@JsonRawValue`로 `graphJson`을 raw JSON으로 직렬화하면 클라이언트는 별도 파싱 불필요. 단, `WorkflowDefinitionDetail.from()`에서 JSON 파싱 검증 후 실패 시 `WORKFLOW_GRAPH_JSON_INVALID`(500) 예외를 발생시킨다 (DB 데이터 정합성 오류 방어; 정상 write path인 spec 231 V1-V6 검증을 통과한 경우엔 발생하지 않는다).
