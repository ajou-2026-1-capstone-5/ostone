...
`pipeline_job_id`는 Airflow DAG run params의 `pipeline_job_id`에서 제공된다. `publish_candidate`가 `StageContext`를 구성할 때는 `context["params"].get("pipeline_job_id")` 또는 같은 의미의 Airflow context 접근을 사용한다.
...
입력 manifest의 key는 upstream callback request 계열과 맞추기 위해 `candidateArtifactPath`를 사용한다. `publish_candidate` 내부 상태, `publish_candidate_result.json`, 출력 `manifest.json` payload에서는 이를 `candidate_artifact_path`로 변환해 저장한다.
...
| `evaluationSummary.passed` | optional. 필드가 있고 `false`이면 callback을 보내지 않고 `BLOCKED_BY_EVALUATION` 결과를 남김 |
...
|-------|-------|------|--------|
| `pipelineJobId` | Spring Backend creates, Airflow receives through DAG params `pipeline_job_id` | Spring `pipeline.pipeline_job.id` 값. callback URL의 `{jobId}`로 사용하며 숫자 문자열이어야 한다. | Spring이 job/workspace/status를 조회하는 기준 |
...
`callbackResults[].responseBody`와 `error.responseBody`는 항상 raw response text string 또는 null이다. JSON parsing이 성공하면 `parsedResponseBody`에 parsed JSON object/array를 저장할 수 있다. JSON parsing이 실패하면 원본 raw text를 `responseBody`에 저장하고 `parsedResponseBody`는 null로 둔다.
...
- response JSON parsing 및 raw response text 보존
...
4. 내부 변수와 출력 artifact key는 `candidate_artifact_path`로 변환
...
| `evaluationSummary.passed = false` | callback 미전송, `BLOCKED_BY_EVALUATION` result 기록. `callbackResults`는 empty, `failedCallbackType`은 null, `error`에는 차단 사유를 기록 |
...
| callback response JSON parse 실패 | `parsedResponseBody = null`, `responseBody = rawText`로 result/error에 기록 |
...
| `test_callback_response_body_keeps_raw_text` | JSON parse 성공/실패와 무관하게 responseBody는 raw string 또는 null |
...
- [ ] `publish_candidate`는 입력 `candidateArtifactPath`를 내부/출력 `candidate_artifact_path`로 변환해 기록한다.
...
- [ ] callback 실패 시 실패 callback type, HTTP status, raw response body string이 result 파일에 남는다.
...
- [ ] evaluation gate 실패 시 `publishStatus = "BLOCKED_BY_EVALUATION"`, empty `callbackResults`, null `failedCallbackType`, 차단 사유가 포함된 `error` 또는 `blockReason`을 남긴다.
...
3. `evaluationSummary.passed = false`일 때 Airflow task를 실패로 볼 것인가, publish skipped 성공으로 볼 것인가?
   - 본 스펙의 권장안: callback은 미전송하고 result에는 `BLOCKED_BY_EVALUATION`을 남긴다. Airflow task 실패 여부는 evaluation/pipeline 운영 정책에서 최종 결정한다.
