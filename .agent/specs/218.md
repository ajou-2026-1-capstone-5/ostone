...
| `evaluationSummary.passed` | optional. 필드가 있고 `false`이면 callback을 보내지 않고 `SKIPPED` 또는 `BLOCKED_BY_EVALUATION` 결과를 남김 |
...
| `evaluationSummary.passed = false` | callback 미전송, `SKIPPED` 또는 `BLOCKED_BY_EVALUATION` result 기록. Airflow task 실패 여부는 운영 정책에 따라 결정 |
