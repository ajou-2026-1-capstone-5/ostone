...
| `domainPackDraft.packKey` | 필수 string, lowercase kebab-case, max 100 |
| `domainPackDraft.packName` | 필수 non-blank string, max 255 |
| `intentDraft.intents` | 1개 이상 |
| `intentDraft.intents[].intentCode` | lowercase snake_case, max 100, candidate 내 중복 금지 |
| `workflowDraft` | `slots/policies/risks/workflows/intentSlotBindings/intentWorkflowBindings` 중 최소 1개 이상 |
| `workflowDraft.*Code` | lowercase snake_case, max 100, 같은 entity type 내 중복 금지 |
...
| `workflowDraft.workflows[].graphJson`의 `policyRef` | 참조하는 `policyCode`가 candidate 안에 존재해야 함 |
...
callback 계약에서 ID/Code 성격의 값은 하드한 규칙을 둔다. 이 값들은 멱등성, DB row 연결, binding 해소에 직접 쓰이기 때문이다.

반대로 `name`, `description`, `summaryJson`, `evidenceJson`, `metaJson`처럼 사람이 읽거나 실험적으로 바뀔 가능성이 높은 값은 소프트하게 둔다. 단, Spring DTO의 길이 제한과 JSON string 형식은 지켜야 한다.

### Hard Contract

| Field | Owner | Rule | Reason |
|-------|-------|------|--------|
| `pipelineJobId` | Spring Backend creates, Airflow receives through DAG params/conf | Spring `pipeline.pipeline_job.id` 값. callback URL의 `{jobId}`로 사용하며 숫자 문자열이어야 한다. | Spring이 job/workspace/status를 조회하는 기준 |
| `externalEventId` | `publish_candidate` | `{dag_id}:{run_id}:{callback_type}`. 255자 초과 시 deterministic fallback. retry마다 동일해야 한다. | webhook 멱등성 key |
...
| `packKey` | upstream candidate producer | lowercase kebab-case. `^[a-z0-9]+(-[a-z0-9]+)*$`, max 100. | workspace 안의 domain pack 식별자 |
| `intentCode` | upstream candidate producer | lowercase snake_case. `^[a-z][a-z0-9_]*$`, max 100, 같은 candidate 안에서 중복 금지. | intent row 식별 및 binding 참조 |
| `slotCode` | upstream candidate producer | lowercase snake_case. `^[a-z][a-z0-9_]*$`, max 100, 같은 candidate 안에서 중복 금지. | slot row 식별 및 binding 참조 |
| `policyCode` | upstream candidate producer | lowercase snake_case. `^[a-z][a-z0-9_]*$`, max 100, 같은 candidate 안에서 중복 금지. | policy row 식별 및 workflow `policyRef` 해소 |
| `riskCode` | upstream candidate producer | lowercase snake_case. `^[a-z][a-z0-9_]*$`, max 100, 같은 candidate 안에서 중복 금지. | risk row 식별 |
| `workflowCode` | upstream candidate producer | lowercase snake_case. `^[a-z][a-z0-9_]*$`, max 100, 같은 candidate 안에서 중복 금지. | workflow row 식별 및 binding 참조 |
