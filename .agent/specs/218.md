...
- `publishContext.*`: callback 진행 중/후 Airflow가 채움
...
| `evaluationSummary` | upstream producer | publish 가능 여부 및 품질 지표 요약 | 중간 |
...
| `evaluationSummary.passed` | `false`이면 callback을 보내지 않고 실패 처리 |
