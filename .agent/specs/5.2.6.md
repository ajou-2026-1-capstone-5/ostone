...
- 같은 `currentState + actionType + edgeId` 조합이 반복되면 즉시 중단한다. `edgeId`는 null 가능하며 null도 distinct value로 취급해 `ASK_SLOT`, `WAIT` 같은 non-edge action도 루프 검출에 포함한다.
...
| `actionType` | 응답 방향 |
| --- | --- |
| `ASK_SLOT` | `missingSlotCodes` 배열의 첫 번째 slot을 자연스럽게 질문한다. 테스트도 이 배열 순서를 우선순위 기준으로 검증한다. |
