...
로컬 개발·검증 환경에서 `docker compose up --build -d` 한 번으로 Airflow 3.1.7 + Python 3.13 기반 파이프라인 실행환경을 초기화하고, DAG 파싱·실행·로그 확인·artifact 저장까지 일관되게 재현할 수 있도록 실행환경 구조와 운영 규칙을 정의한다.
...
- 현재 저장소에는 `ml/airflow/` 디렉터리가 없으므로, 이 경로는 **이번 스펙에서 새로 생성할 예정인 런타임 전용 디렉터리**로 취급한다.
...
- 현재 저장소에는 `ml/dags/` 디렉터리가 존재하지 않는다.
- 따라서 `./ml/dags:/opt/airflow/dags` 마운트는 **Airflow DAG 디렉터리 생성 이후 적용할 예정인 목표 상태**다.
- 현재 존재하는 경로 중 이번 스펙에서 바로 활용 가능한 것은 `./ml/src:/opt/airflow/src` 마운트다.
...
따라서 현재 기준으로는 `ml/src/pipeline/...` import 구조는 유지 가능하지만, `ml/dags/`와 `ml/airflow/`는 **신규 생성 예정 경로**로 취급해야 한다.
