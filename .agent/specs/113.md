...
로컬 개발·검증 환경에서 `docker compose up -d` 한 번으로 Airflow 3.1.7 + Python 3.13 기반 파이프라인 실행환경을 초기화하고, DAG 파싱·실행·로그 확인·artifact 저장까지 일관되게 재현할 수 있도록 실행환경 구조와 운영 규칙을 정의한다.
...
> **Latest known release at spec time**: `3.2.0` — 로컬 개발 안정성과 Python 3.13 계약 정렬을 위해 의도적으로 `3.1.7`에 고정한다.
...
`docker compose up -d` 한 번으로 끝내기 위해 아래 조건을 명시한다.
...
- `backend`와 `frontend`는 멀티스테이지 Dockerfile을 사용해 선행 산출물 없이도 빌드 가능하다.
- Dockerfile/requirements 변경이 있을 때만 `--build`로 강제 재생성하면 충분하다.

### Airflow 이미지 정책

- 베이스 이미지: `apache/airflow:3.1.7-python3.13`
- 커스텀 이미지는 **항상 이 버전 위에서 확장**
- `추가 예정: ml/airflow/requirements.txt` 설치 시 `apache-airflow==3.1.7`을 함께 명시해 버전 드리프트를 막는다.
...
- `추가 예정: ./ml/dags:/opt/airflow/dags`
...
- `ml/dags/`와 `ml/src/`는 모두 저장소 경로로 존재하며, compose에서 직접 bind mount 한다.
...
- [Airflow 3.1.7 Docker Compose](https://airflow.apache.org/docs/apache-airflow/3.1.7/howto/docker-compose/index.html)
- [Airflow Docker image build guide](https://airflow.apache.org/docs/docker-stack/build.html)
- [Airflow 3.1.7 Configuration Reference](https://airflow.apache.org/docs/apache-airflow/3.1.7/configurations-ref.html)
- [Airflow 3.1.7 Modules Management](https://airflow.apache.org/docs/apache-airflow/3.1.7/administration-and-deployment/modules_management.html)
- [Airflow 3.1.7 DAG File Processing](https://airflow.apache.org/docs/apache-airflow/3.1.7/administration-and-deployment/dagfile-processing.html)
