...
flowchart LR
    ...
    AS --> DAGS["runtime DAG: ml/src/pipeline/dags/"]
...
- `ml/src/pipeline/dags/`는 Airflow runtime orchestration entrypoint
...
- `추가 예정: ml/airflow/requirements.txt` 설치 시 `apache-airflow==3.2.0`을 함께 명시해 버전 드리프트를 막는다.
...
- `./ml/src/pipeline/dags:/opt/airflow/dags`
...
- `ml/src/pipeline/dags/`와 `ml/src/`는 compose에서 직접 bind mount 한다.
