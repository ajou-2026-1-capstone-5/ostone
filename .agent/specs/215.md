...
- `QUEUED` 상태도 terminal failure callback 처리 대상에 포함하므로, `airflowDagId`, `airflowRunId`는 `QUEUED` 생성 transaction 안에서 반드시 저장되어 있어야 한다.
...
`pipeline_job`에는 아래 값을 반영한다. `airflowDagId`, `airflowRunId`는 deterministic 값이므로 Airflow 호출 전 `QUEUED` 생성 transaction 안에서 저장한다. 이는 Airflow DAG run이 이미 생성되었지만 Spring의 trigger 응답 처리 또는 `RUNNING` 전이 transaction이 지연/실패한 상황에서도 terminal failure callback의 target validation을 수행하기 위한 전제다.
