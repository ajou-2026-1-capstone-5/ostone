...
이 API는 version lifecycle의 최종 방어선이다. Published version을 편집하려는 UX는 먼저 revision draft를 생성해 DRAFT version으로 이동한 뒤 이 PATCH를 호출해야 한다. 이미 DRAFT version을 보고 있는 경우에는 새 draft를 만들지 않고 현재 workflow row에 대해 바로 PATCH를 호출한다.

## Frontend Orchestration Contract

| 화면 상태 | FE 동작 | BE 기대 |
| --- | --- | --- |
| 선택 version이 `DRAFT` | 현재 `workflowId`로 PATCH 호출 | 200 또는 graph/name 검증 오류 |
| 선택 version이 `PUBLISHED` | PATCH 직접 호출 금지, 먼저 `POST /revision-drafts` 호출 | PATCH 직접 호출 시 `WORKFLOW_NOT_EDITABLE` |
| revision draft 생성 성공 | 복제된 DRAFT version에서 동일 `workflowCode`의 새 `workflowId`를 찾아 이동 | clone은 workflow business key인 `workflowCode`를 보존 |
| 기존 DRAFT 존재 | FE가 기존 DRAFT로 이동할지 사용자에게 안내 | draft 중복 생성 요청은 `DOMAIN_PACK_DRAFT_ALREADY_EXISTS` |

`WORKFLOW_NOT_EDITABLE`은 정상 UX의 주 분기가 아니라 잘못된 클라이언트 호출, 오래된 화면 상태, 수동 요청을 막는 방어적 오류다.
...
sequenceDiagram
    ...
    note over uc: V1–V8c 위반 시 해당 예외 throw (propagate to GlobalExceptionHandler)
...
```json
{
  ...
  "graphJson": {
    ...
    "edges": [
      {"id": "e_start_to_n1", "from": "start", "to": "n1",  "label": null},
      {"id": "e_n1_to_end",   "from": "n1",    "to": "end", "label": null}
    ]
  }
}
```
...
- `edges[]`: 각 원소에 `id`(String, V7a 필수), `from`(String), `to`(String), `label`(String, nullable) 포함
- V1–V8b 그래프 검증 (`WorkflowGraphValidator.parseAndValidate()`) 적용
- V8c policyRef 존재 여부 검증 (`DomainPackValidator.validatePolicyCodes()`) 적용
- 크기 제한: 직렬화 후 문자 수 ≤ 100,000자 (≈ 100KB). UseCase 상수 `MAX_GRAPH_JSON_CHARS = 100_000`
...
| `VALIDATION_ERROR` | name 비어 있음 / graphJson 누락 / domain method 검증 실패 |
| `GRAPH_JSON_TOO_LARGE` | graphJson 크기 초과 |
| `DOMAIN_PACK_DRAFT_INVALID_REQUEST` | graphJson JSON 파싱 실패 또는 node id/type 누락 |
| `WORKFLOW_INVALID_START_NODE` | V1 위반: START 노드 ≠ 1개 |
| `WORKFLOW_INVALID_TERMINAL_NODE` | V2 위반: TERMINAL 노드 0개 |
| `WORKFLOW_DANGLING_EDGE` | V3 위반: 존재하지 않는 노드 참조 |
| `WORKFLOW_UNREACHABLE_NODE` | V4 위반: START에서 미도달 노드 |
| `WORKFLOW_CYCLE_DETECTED` | V5 위반: 사이클 검출 |
| `WORKFLOW_UNLABELED_BRANCH` | V6 위반: DECISION outgoing edge 레이블 없음 |
| `WORKFLOW_EDGE_ID_MISSING` | V7a 위반: edge id 누락 |
| `WORKFLOW_EDGE_ID_DUPLICATE` | V7b 위반: edge id 중복 |
| `WORKFLOW_ACTION_NODE_POLICY_REF_MISSING` | V8a 위반: ACTION node policyRef 누락 |
| `WORKFLOW_ACTION_NODE_POLICY_REF_INVALID_CHARS` | V8b 위반: ACTION node policyRef 형식 불일치 |
| `WORKFLOW_ACTION_NODE_POLICY_REF_NOT_FOUND` | V8c 위반: ACTION node policyRef가 version 내 policy code로 존재하지 않음 |
    ...
    mockMvc.perform(patch("/api/v1/workspaces/1/domain-packs/1/versions/1/workflows/10")
            ...
            .content("""
                {"name":"수정명","description":null,
                 "graphJson":{"direction":"LR",
                   "nodes":[
                     {"id":"start","type":"START"},
                     {"id":"end","type":"TERMINAL"}
                   ],
                   "edges":[
                     {"id":"e_start_to_end","from":"start","to":"end","label":null}
                   ]}}
                """))
...
- [ ] graphJson V1 위반 (START 노드 != 1) → 400 `WORKFLOW_INVALID_START_NODE`
- [ ] graphJson V2 위반 (TERMINAL 노드 없음) → 400 `WORKFLOW_INVALID_TERMINAL_NODE`
- [ ] graphJson V3 위반 (dangling edge) → 400 `WORKFLOW_DANGLING_EDGE`
- [ ] graphJson V4 위반 (미도달 노드) → 400 `WORKFLOW_UNREACHABLE_NODE`
- [ ] graphJson V5 위반 (사이클) → 400 `WORKFLOW_CYCLE_DETECTED`
- [ ] graphJson V6 위반 (DECISION label 없음) → 400 `WORKFLOW_UNLABELED_BRANCH`
- [ ] graphJson V7a 위반 (edge id 없음) → 400 `WORKFLOW_EDGE_ID_MISSING`
- [ ] graphJson V7b 위반 (edge id 중복) → 400 `WORKFLOW_EDGE_ID_DUPLICATE`
- [ ] graphJson V8a 위반 (ACTION policyRef 없음) → 400 `WORKFLOW_ACTION_NODE_POLICY_REF_MISSING`
- [ ] graphJson V8b 위반 (ACTION policyRef 형식 불일치) → 400 `WORKFLOW_ACTION_NODE_POLICY_REF_INVALID_CHARS`
- [ ] graphJson V8c 위반 (ACTION policyRef가 version 내 policy code가 아님) → 400 `WORKFLOW_ACTION_NODE_POLICY_REF_NOT_FOUND`
...
- V1–V8c 예외는 `GlobalExceptionHandler`의 `BusinessException` fallback (400)으로 처리됨.
...
- graphJson 크기 제한: 직렬화 후 100,000자(≈ 100KB) 이하. `UpdateWorkflowUseCase.MAX_GRAPH_JSON_CHARS = 100_000` 상수로 관리.
