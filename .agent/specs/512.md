...
선택한 version이 `DRAFT`인 경우 Domain Pack summary 화면에서 draft 적용(activate)과 폐기(discard) 액션을 제공한다. Workflow/Intent 편집 화면은 draft 내부 row를 수정하고, draft 전체를 운영 version으로 반영하거나 버리는 결정은 Domain Pack 화면에서 수행한다.
...
flowchart TD
    ...
    H --> N{"선택 version = DRAFT?"}
    N -->|Yes| O["Domain Pack summary에서 적용/폐기 액션"]
    O --> P["적용: activate 후 새 운영 version 조회"]
    O --> Q["폐기: draft 삭제 후 운영 version 또는 summary로 이동"]
...
- `Summary JSON`은 유지한다.
- 선택 version이 `DRAFT`이면 Domain Pack summary/detail 영역에 `적용`과 `폐기` 액션을 노출한다.
- draft `적용`은 기존 activate API를 호출하고, 성공 후 적용된 version context를 유지한다.
- draft `폐기`는 discard API를 호출하고, 성공 후 현재 운영 version 또는 pack summary로 이동한다.
...
| DRAFT version action | 적용/폐기 진입점 불명확 | Domain Pack summary/detail에서 `적용`/`폐기` 제공 |
...
6. 선택 version이 `DRAFT`이면 Domain Pack summary/detail에 draft action bar를 표시한다.
7. `적용` 클릭 시 `POST /versions/{draftVersionId}/activate`를 호출하고 상세 데이터를 refetch한다.
8. `폐기` 클릭 시 사용자 확인 후 `DELETE /versions/{draftVersionId}/draft`를 호출하고, 현재 운영 version이 있으면 그 version으로 `replace`, 없으면 pack summary 기본 상태로 이동한다.
...
- draft 적용/폐기 mutation 성공 후 pack detail, pack list, component list/detail query를 무효화한다.
- draft 폐기 성공 후 삭제된 `versionId`가 URL에 남지 않도록 즉시 `replace` navigation을 수행한다.
...
| draft 적용 | 적용된 `?versionId=...` 유지 또는 최신 운영 version으로 replace | replace | 적용은 version context 갱신이며 별도 화면 이동이 아님 |
| draft 폐기 | 현재 운영 version `?versionId=...` 또는 query 제거 | replace | 삭제된 draft version URL을 history에 남기지 않음 |
...
| draft version 선택 | 선택 version lifecycle이 `DRAFT` | 요약/detail 상단 또는 하단에 `적용`/`폐기` 액션 표시 |
| draft 적용 성공 | activate mutation 성공 | pack detail/list refetch, 적용된 version context 유지 |
| draft 폐기 성공 | discard mutation 성공 | 삭제된 draft version context 제거, 현재 운영 version 또는 summary 기본 상태로 replace |
...
| `승인 준비 상태` | 제거 |
| `Summary JSON` | 유지 |
| activate/readiness UI | readiness UI는 노출하지 않음 |
| DRAFT action bar | 선택 version이 `DRAFT`일 때만 `적용`/`폐기` 버튼 노출 |
...
| `frontend/src/features/domain-pack-summary-read/ui/SummaryDetailPanel.tsx` | modify | DRAFT version 적용/폐기 action bar 추가 |
...
| DRAFT version 선택 | Summary detail에 `적용`/`폐기` 액션이 노출됨 |
| DRAFT 적용 성공 | activate 후 적용된 version context가 유지되고 pack detail/list가 갱신됨 |
| DRAFT 폐기 성공 | draft 삭제 후 삭제된 `versionId`가 URL에 남지 않음 |
