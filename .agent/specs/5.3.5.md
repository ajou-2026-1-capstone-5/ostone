...
| PATCH 또는 기존 `consultationApi.updateStatus(sessionId, 'COMPLETED')` 경로 | 상담 종료 처리 |
...
백엔드 5.3.2 스펙 파일은 `/Users/owen/Projects/worktree/ostone-spec-535/.agent/specs/5.3.2.md`에서 확인되지 않았다. 따라서 상담사 메시지 발행 destination은 구현 전에 백엔드 스펙 또는 실제 message mapping으로 확정해야 한다. 이 문서에서는 구독 destination만 확정한다.
