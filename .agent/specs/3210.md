...
| `transitionId` | String | graphJson edge id |

Headers:

- `Authorization: Bearer {jwt-token}` 필수

Body는 `condition`, `action`, `outcome` 중 최소 1개를 포함한다. 없는 섹션은 기존 값을 유지한다.
...
| code | 원인 |
|------|------|
| `VALIDATION_ERROR` | Bean Validation 실패 또는 기본 request 형식 오류 |
...
| `DOMAIN_PACK_VERSION_NOT_FOUND` | version 미존재 |
| `WORKFLOW_DEFINITION_NOT_FOUND` | workflow 미존재 |
...
sequenceDiagram
    ...
    db -->> wr: workflow
    uc ->> uc: parse graphJson and find edge by transitionId
    uc ->> uc: resolve fromNode and toNode
    uc ->> uc: apply condition/action/outcome patch
    opt action.policyRef changed
        uc ->> val: validatePolicyCodes(versionId, Set(policyRef))
        val -->> uc: ok
    end
...
| `WorkflowTransitionDetail.java` | `fromType`, `toType`, `condition`, `action`, `outcome` 필드 추가 |
...
1. `edges[]`에서 `id == transitionId`인 edge를 찾는다.
2. edge의 `from`, `to`로 `nodes[]`에서 fromNode/toNode를 찾는다.
3. request에 `condition`이 있으면 fromNode type이 `DECISION`인지 확인하고 edge `label`을 수정한다.
4. request에 `action`이 있으면 toNode type이 `ACTION`인지 확인하고 toNode `policyRef`를 수정한다.
5. request에 `outcome`이 있으면 toNode type이 `TERMINAL`인지 확인하고 toNode `state`/`label`을 보낸 필드만 수정한다.
6. 수정된 graphJson 전체를 문자열로 직렬화한다.
7. `WorkflowGraphValidator.parseAndValidate(...)`를 재실행한다.
8. `WorkflowGraphValidator.extractInitialState(...)`, `extractTerminalStatesJson(...)`로 derived column을 갱신한다.
9. `workflow.updateGraph(existingName, existingDescription, updatedGraphJson, initialState, terminalStatesJson)`를 호출한다.
...
- [ ] request의 `condition/action/outcome` 중 일부만 보내도 command에 null 유지 전달
...
- [ ] ACTION `policyRef`는 기존 V8 규칙과 version 내부 policyCode 존재 검증을 따른다.
