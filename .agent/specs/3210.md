...
| `transitionId` | String | graphJson edge id. URL-safe 문자만 허용 (`[A-Za-z0-9_-]+`) |
...
- `transitionId`는 `[A-Za-z0-9_-]+` 패턴을 만족해야 한다. 이는 선행 transition READ 스펙의 edge id URL-safe 규칙과 동일하다.
...
| code | 원인 |
|------|------|
| `VALIDATION_ERROR` | Bean Validation 실패, malformed JSON, empty body, 명시적 null, transitionId 패턴 위반 등 기본 request 형식 오류 |
...
sequenceDiagram
    ...
    uc ->> uc: find edge by transitionId
...
2. `transitionId`가 `[A-Za-z0-9_-]+` 패턴을 만족하는지 확인한다.
3. `edges[]`에서 `transitionId`와 `id`가 exact match인 edge를 찾는다.
4. edge의 `from`, `to`로 `nodes[]`에서 fromNode/toNode를 찾는다.
5. request에 `condition`이 있으면 fromNode type이 `DECISION`인지 확인하고 edge `label`을 수정한다.
6. request에 `action`이 있으면 toNode type이 `ACTION`인지 확인하고 toNode `policyRef`를 수정한다.
7. request에 `outcome`이 있으면 toNode type이 `TERMINAL`인지 확인하고 toNode `state`/`label`을 보낸 필드만 수정한다.
8. 수정된 graphJson 전체를 문자열로 직렬화한다.
9. `WorkflowGraphValidator.parseAndValidate(...)`를 재실행한다.
10. 수정 후 graphJson 전체의 ACTION node `policyRef`를 수집해 `DomainPackValidator.validatePolicyCodes(...)`로 version 내부 policyCode 존재 여부를 검증한다.
11. `WorkflowGraphValidator.extractInitialState(...)`, `extractTerminalStatesJson(...)`로 derived column을 갱신한다.
12. `workflow.updateGraph(existingName, existingDescription, updatedGraphJson, initialState, terminalStatesJson)`를 호출한다.

PATCH는 transition `id`가 없는 legacy edge를 수정 대상으로 지원하지 않는다. 현재 graphJson이 기존 검증 규칙을 통과하지 못하면, 요청한 필드 자체가 유효하더라도 PATCH는 실패한다.
...
- [ ] `transitionId`는 선행 transition READ 스펙과 동일하게 URL-safe 패턴(`[A-Za-z0-9_-]+`)만 허용한다.
...
- [ ] DB schema migration 없이 기존 `workflow_definition` row의 `graph_json` 및 derived columns만 갱신한다.
...
- `outcome.state`는 현재 runtime 실행 제어와 강하게 연결되어 있지 않다. 현재 구현에서는 terminal states가 node `state`가 아니라 TERMINAL node `id` 기준으로 추출된다. 따라서 `outcome.state` 변경 자체가 현재 runtime state 전이 또는 `terminal_states_json` 산출 기준을 바꾸지는 않는다.
...
- 이 API는 작은 의미 필드 PATCH지만 저장 전후 전체 graph validation을 다시 수행한다. 기존에 저장된 graphJson이 이미 invalid하면 요청 필드가 정상이어도 실패할 수 있다.
