...
> **Depends on**: `.agent/specs/213.md`(intent-drafts callback), `.agent/specs/226.md`(graphJson V1~V6 정의), `.agent/specs/2217.md`(graphJson V7 edge id 정의)
...
- `DomainPackDraftPersistenceService` 에 `persistWorkflowDraft(...)` 신규 메서드 추가
  ...
  - graphJson 은 `WorkflowGraphValidator.parseAndValidate(...)` 로 V1~V8 검증 후 `initialState` / `terminalStatesJson` 추출
...
| `*.name` | 필수, 최대 255자 |
| `*Json` (jsonb 컬럼) | 선택, 최대 20000자 (각 row 별) |
...
sequenceDiagram
    ...
    draftSvc ->> db: WorkflowGraphValidator.parseAndValidate (V1~V8) per workflow
...
public class ReceiveWorkflowDraftCallbackUseCase {
  ...
  public ReceiveWorkflowDraftCallbackResult execute(ReceiveWorkflowDraftCallbackCommand command) {
    ...
    // payload 원문 저장 (213 의 domain-pack-drafts callback 과 같은 패턴)
...
3. 각 workflow 는 `WorkflowGraphValidator.parseAndValidate(...)` (V1~V8) 로 graphJson 검증 + `initialState` / `terminalStatesJson` 추출.
4. ACTION 노드의 `policyRef` 가 **이번 callback 에서 들어온 policies** 또는 **DB 에 이미 존재하는 같은 version 의 policy_code** 둘 중 한 곳에서 발견되어야 함 — 둘 다 없으면 `WORKFLOW_ACTION_NODE_POLICY_REF_NOT_FOUND` (기존 예외 재사용).
...
- `pipeline_artifact` insert 와 `webhook_receipt` insert 는 별도 트랜잭션 (`TransactionTemplate.execute(...)`) 으로 처리해, persist 단계 실패 시에도 receipt 가 `FAILED` 로 남도록 한다 (213 의 `markFailure(...)` 패턴 그대로).
...
- graphJson V1~V8 위반 (각 케이스 1 개씩) → 해당 예외 코드
...
- `pipeline_artifact` row 가 callback 1 회당 1 row 생성됨 (duplicate ignored 시에는 0 row)
...
- 400 graphJson V1~V8 위반 (`WORKFLOW_CYCLE_DETECTED` 응답 코드 검증)
...
- [ ] graphJson 검증: V1~V8 각 위반 케이스에 대응되는 에러 코드
...
- workflow graphJson 의 V1~V8 검증은 본 스펙에서 새로 만들지 않고, 기존 `WorkflowGraphValidator.parseAndValidate(...)` 와 그 예외 클래스를 그대로 재사용한다. 이는 `CreateDomainPackDraftUseCase`(spec 231) 의 write path 와 동일한 의미를 가지도록 보장한다.
