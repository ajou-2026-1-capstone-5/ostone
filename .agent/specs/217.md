...
- `webhook_receipt` 기반 멱등 처리 (213 와 동일 패턴, `externalEventId` 단독 idempotency key 유지)
  ...
  - 동일 `externalEventId` receipt 가 이미 존재하지만 `webhook_type` 이 `WORKFLOW_DRAFT_CALLBACK` 이 아니면 duplicate 가 아니라 409 conflict 로 처리한다
...
| `*Json` (jsonb 컬럼, `workflows[].graphJson` 제외) | 선택, 최대 5000자 (각 필드 별) |
...
동일 `externalEventId` 와 동일 `webhook_type = WORKFLOW_DRAFT_CALLBACK` 의
`webhook_receipt.processing_status = PROCESSED` 재요청.
...
**409 Conflict — externalEventId 가 다른 callback type 에서 이미 사용됨**

이 오류는 Airflow 가 동일 `externalEventId` 를 서로 다른 callback type 에 재사용한 계약 위반이다.
동일 payload 를 재시도해도 같은 409 가 반환되므로 자동 재시도 대상이 아니며,
Airflow 의 `externalEventId` 생성 / callback 매핑을 수정한 뒤 새 `externalEventId` 로 다시 전송해야 한다.

```json
{ "code": "WEBHOOK_RECEIPT_TYPE_CONFLICT", "message": "externalEventId가 다른 webhook type으로 이미 수신되었습니다. externalEventId=evt-workflow-1, existingType=INTENT_DRAFT_CALLBACK, requestedType=WORKFLOW_DRAFT_CALLBACK" }
```
...
sequenceDiagram
    ...
    alt receipt exists && receipt.webhook_type != WORKFLOW_DRAFT_CALLBACK
        cb -->> ctrl: 409 WEBHOOK_RECEIPT_TYPE_CONFLICT
    else receipt.webhook_type == WORKFLOW_DRAFT_CALLBACK && receipt.processing_status == PROCESSED
...
| `WebhookReceiptTypeConflictException.java` | `pipelinejob/application/exception/` | 동일 `externalEventId` 가 다른 `webhook_type` 으로 이미 수신된 경우 409 응답 |
...
| `pipelinejob/domain/model/WebhookReceipt.java` | `webhook_type` 충돌 검증을 위해 `getWebhookType()` getter 추가. schema / unique constraint 는 변경하지 않는다. |
...
public class ReceiveWorkflowDraftCallbackUseCase {
  ...
  public ReceiveWorkflowDraftCallbackResult execute(ReceiveWorkflowDraftCallbackCommand command) {
    ...
    validateWebhookType(existing.orElse(null), command.externalEventId());
  ...
  // 단, 조회된 receipt.webhookType 이 WEBHOOK_TYPE 과 다르면
  // WebhookReceiptTypeConflictException 을 던져 409 로 처리한다.
...
- 조회된 receipt 의 `webhook_type` 이 현재 callback type(`WORKFLOW_DRAFT_CALLBACK`) 과 다르면, `processing_status` 와 무관하게 `WEBHOOK_RECEIPT_TYPE_CONFLICT` 409 로 거부한다.
- 현재 callback type 과 동일한 `PROCESSED` receipt 만 duplicate 로 간주하고, 동일 type 의 `FAILED` / `RECEIVED` 는 재처리 대상이다.
...
- duplicate ignored (동일 `externalEventId`, 동일 `webhook_type = WORKFLOW_DRAFT_CALLBACK` 의 `PROCESSED` receipt)
- 동일 `externalEventId` 이지만 다른 `webhook_type` receipt 가 이미 존재하면 → `WEBHOOK_RECEIPT_TYPE_CONFLICT`
- `FAILED` receipt 재처리 성공
- receipt insert 충돌 후 `externalEventId` 재조회 결과가 동일 `webhook_type` 의 `PROCESSED` receipt 이면 duplicate 처리
- receipt insert 충돌 후 `externalEventId` 재조회 결과가 다른 `webhook_type` 이면 → `WEBHOOK_RECEIPT_TYPE_CONFLICT`
...
- 201 정상
- 200 duplicate ignored (동일 `externalEventId`, 동일 `webhook_type`)
...
- 409 receipt type conflict (`externalEventId` 가 다른 `webhook_type` 으로 이미 수신됨)

### Test Checklist

- [ ] 정상 시나리오: workflow callback 1 회로 slot/policy/risk/workflow + binding 모두 적재 + `SUCCEEDED` 종료
- [ ] 멱등성: 동일 `externalEventId` + 동일 `webhook_type` 재요청 시 duplicate ignored
- [ ] 멱등성: 동일 `externalEventId` + 다른 `webhook_type` 이면 `processing_status` 와 무관하게 409 `WEBHOOK_RECEIPT_TYPE_CONFLICT`
- [ ] 권한: secret 불일치 시 401
...
- [ ] 동시성: 같은 job 에 동일한 `externalEventId` 를 가진 workflow callback 2 개가 동시에 들어오면 → 먼저 처리 완료된 요청은 201, 이후 재요청은 200 `DUPLICATE_IGNORED` (동시 insert 충돌 시 `externalEventId` 재조회 후 `webhook_type` 과 processed 여부에 따라 duplicate 또는 conflict 처리)
...
| 테이블 | 비고 |
|--------|------|
| `pack.slot_definition` | 기존 (`backend/src/main/resources/db/changelog/db.changelog-master.sql:163-179`, verified) |
| `pack.intent_slot_binding` | 기존 (`backend/src/main/resources/db/changelog/db.changelog-master.sql:181-192`, verified) |
| `pack.policy_definition` | 기존 (`backend/src/main/resources/db/changelog/db.changelog-master.sql:194-210`, verified) |
| `pack.risk_definition` | 기존 (`backend/src/main/resources/db/changelog/db.changelog-master.sql:212-228`, verified) |
| `pack.workflow_definition` | 기존 (`backend/src/main/resources/db/changelog/db.changelog-master.sql:230-246`, verified) |
