...
```json
{
  ...
  "message": "허용되지 않은 상태 값입니다: REJECTED. 허용 값: PUBLISHED, REJECTED"
}
```
...
class UpdateIntentStatusControllerTest {
    ...
    void updateStatus_withInvalidStatus_returns400() throws Exception {
        mockMvc.perform(patch("/api/v1/workspaces/1/domain-packs/10/versions/100/intents/42/status")
            ...
            .andExpect(jsonPath("$.code").value("INVALID_STATUS_VALUE"));
    }
}
```
...
참고: `status` 컬럼은 이미 존재하며 기본값이 변경될 수 있다.
...
- 상수 추가: `STATUS_PUBLISHED = "PUBLISHED"`, `STATUS_REJECTED = "REJECTED"`
...
public void changeStatus(String newStatus) {
    if (!STATUS_PUBLISHED.equals(newStatus) && !STATUS_REJECTED.equals(newStatus)) {
        throw new IllegalArgumentException("허용되지 않은 상태 값입니다: " + newStatus);
    }
...
- 상태 전이: DRAFT → PUBLISHED (승인), DRAFT → REJECTED (반려)
