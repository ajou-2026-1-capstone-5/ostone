...
flowchart TD
    ...
    Q -->|삭제 버튼\nOWNER| Y[ArchiveWorkspaceDialog 열기]
...
└── DashboardLayout (shared/ui/layout)
    └── main content:
        ...
        │       ├── Button ("취소", variant=secondary)
        │       └── Button ("저장", isLoading)
        └── ArchiveWorkspaceDialog (UI label: "삭제") — AlertDialog (shared/ui/alert-dialog.tsx) 활용
...
│  │ Dialog            │ │ Dialog            │             │
│  ├──────────────────┤ ├──────────────────┤             │
│  │ ArchiveWorkspaceDialog │ │ WorkspaceList  │             │
│  │ (UI label: 삭제)       │ │ (Card grid)    │             │
│  └──────────────────┘ └──────────────────┘             │
└─────────────────────────────────────────────────────────┘
                           │
                           ▼
...
| `frontend/src/features/archive-workspace/ui/ArchiveWorkspaceDialog.tsx` | new | 삭제 확인 AlertDialog |
...
| 역할 뱃지 | `badge.tsx` |
| 빈 상태 | `empty.tsx` |
| 텍스트 입력 | `input.tsx` (커스텀) / `input/Input.tsx` (아이콘 지원) — `input/Input.tsx`는 아이콘 지원이 필요할 때 사용, `input.tsx`는 단순/커스텀 입력에 사용 |
| 여러 줄 입력 | `textarea.tsx` |
| 버튼 | `button.tsx` / `button/Button.tsx` — `button/Button.tsx`는 아이콘 지원 버튼에 사용, `button.tsx`는 단순/커스텀 버튼에 사용 |
| 로딩 표시 | `spinner.tsx` |
| Toast 알림 | `sonner.tsx` |
