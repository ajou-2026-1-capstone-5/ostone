...
sequenceDiagram
    ...
    Note over conv: null → NullPointerException
    Note over conv: blank / invalid JSON → IllegalArgumentException
    conv -->> svc: ok
...
public class Conversation {
    ...
    // metaJson 갱신: null → NullPointerException; blank / invalid JSON → IllegalArgumentException
...
// ConversationTest.java 구현 완료 (Test Checklist의 [x] 항목 참조)
...
class ConversationTest {
    ...
    void should_IAE_when_languageCodeExceeds20() {
        ...
        assertThatThrownBy(() -> Conversation.create(1L, null, null, tooLong, null, null, null, null, 0))
            .isInstanceOf(IllegalArgumentException.class);
    }
...
- [x] Conversation.create: 유효 입력 → 정상 생성
- [x] Conversation.create: datasetId null → NullPointerException
- [x] Conversation.create: turnCount 음수 → IllegalArgumentException
- [x] Conversation.create: endedAt < startedAt → IllegalArgumentException
- [x] Conversation.create: externalCaseId 255자 초과 → IllegalArgumentException
- [x] Conversation.create: channel 50자 초과 → IllegalArgumentException
- [x] Conversation.create: languageCode 20자 초과 → IllegalArgumentException
- [x] Conversation.create: languageCode null → 기본값 "ko" 적용
- [x] Conversation.create: languageCode blank → 기본값 "ko" 적용
- [x] Conversation.updateMetaJson: blank → IllegalArgumentException
- [x] Conversation.updateMetaJson: 유효하지 않은 JSON → IllegalArgumentException
- [x] Conversation.updateMetaJson: null → NullPointerException
...
- `Conversation`의 노출 getter: `getId()`, `getDatasetId()`, `getTurnCount()`, `getLanguageCode()` 4개. `getLanguageCode()`는 `languageCode` 기본값 "ko" 단위 테스트 검증을 위해 추가됨
- 도메인 엔티티 단위 테스트(`ConversationTest`) 구현 완료. `testing.md` 기준 도메인 로직 커버리지 충족
