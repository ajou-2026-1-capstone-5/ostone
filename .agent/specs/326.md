...
public void updateFields(
  ...
  this.validationRuleJson = validationRuleJson != null ? validationRuleJson : "{}";
...
public void changeStatus(String newStatus) {
  if (!"ACTIVE".equals(newStatus) && !"INACTIVE".equals(newStatus)) {
    throw new BadRequestException("허용되지 않는 status 값입니다: " + newStatus);
  }
...
public class UpdateSlotUseCase {
  ...
  public SlotDefinitionResponse execute(UpdateSlotCommand command) {
    ...
    if (!DomainPackVersion.STATUS_DRAFT.equals(version.getLifecycleStatus())) {
      throw new BadRequestException("DRAFT 상태의 버전에서만 슬롯을 수정할 수 있습니다.", "SLOT_NOT_EDITABLE");
    }
...
- `BadRequestException`에 에러 코드(`SLOT_NOT_EDITABLE`)를 전달할 수 있는지는 기존 `BadRequestException` 구현을 확인한 후 결정한다. 지원하지 않으면 메시지로 구분하거나 별도 예외 클래스를 추가한다.
