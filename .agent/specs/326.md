...
sequenceDiagram
    ...
    participant auth as WorkspaceMembershipPort
    ...
    ctrl ->> auth: validateMembership(workspaceId, requesterId)
    auth -->> ctrl: 403 Forbidden (if no membership)
...
- [ ] Slot Status 전환: 워크스페이스 멤버십 없을 때 403 검증 (/status)
