...
public class UpdateWorkspaceUseCase {
  ...
  public WorkspaceResult execute(UpdateWorkspaceCommand command) {
    ...
    try {
      ...
      // 도메인 validation 실패 → 사용자 친화적인 한글 메시지로 변환
      // (영문 도메인 메시지 ex.getMessage() 노출 방지)
...
- `should_400WORKSPACE_INVALID_NAME_when_name_blank` — `{"name":"   "}` 전송 → 400 (도메인 validation)
- `should_400WORKSPACE_INVALID_NAME_when_name_null_전달` — `{"name":null}` 전송 → 400 (도메인 validation)
- `should_400WORKSPACE_INVALID_NAME_when_name_255초과` — `{"name":"x...x(256자)"}` 전송 → 400 (도메인 validation: workspace.update() → IAE → WorkspaceInvalidNameException)
