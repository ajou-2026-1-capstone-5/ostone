...
> `UpdateWorkspaceRequest`는 두 필드를 `Optional<String>`으로 선언한다. absent 필드 → `Optional.empty()` → UseCase에서 현재 값 유지. present 필드 → `Optional.of(value)` → 변경. blank-only `name`이 전달되면 도메인 `Workspace.update()`가 `IllegalArgumentException`을 던지고 UseCase가 이를 `WorkspaceInvalidNameException`(400)으로 변환한다.
...
public class UpdateWorkspaceUseCase {
  ...
  public WorkspaceResult execute(UpdateWorkspaceCommand command) {
    ...
    // Optional.empty() = 미전달 → 현재 값 유지, Optional.of(v) = 명시적 전달 → v로 변경
    String newName = command.name().orElse(workspace.getName());
    String newDescription = command.description().isPresent()
...
public class WorkspaceController {
  ...
  // UpdateWorkspaceRequest.name/description 은 Optional<String>이므로
  // absent 필드는 Optional.empty() → UseCase에서 현재 값 유지, present 필드만 변경된다.
...
// Optional.empty() = 필드 미전달 (변경 없음), Optional.of(...) = 명시적 전달 (변경)
// Jackson 설정: @JsonSetter(nulls = Nulls.AS_EMPTY) 또는 커스텀 역직렬화로 absent → Optional.empty()
public record UpdateWorkspaceRequest(
    Optional<@Size(max = 255) String> name,
    Optional<@Size(max = 2000) String> description) {

  // canonical constructor — absent 시 Optional.empty() 보장
...
// 수정 커맨드 — Optional.empty() = 미전달(변경 없음), Optional.of(value) = 명시적 전달
public record UpdateWorkspaceCommand(
  ...
  public UpdateWorkspaceCommand {
    ...
    Objects.requireNonNull(name, "name optional must not be null");
    Objects.requireNonNull(description, "description optional must not be null");
  }
}
...
- `should_name과_description_수정_when_OWNER`
  - given: `name = Optional.of("new")`, `description = Optional.of("desc")`
  - then: workspace.update("new", "desc") 호출 확인
- `should_name만_수정_when_description_absent`
  - given: `name = Optional.of("new")`, `description = Optional.empty()`
  - then: description은 기존 값 유지 (`workspace.getDescription()` 값으로 `update` 호출)
- `should_description_absent_시_기존값_유지`
  - given: `name = Optional.empty()`, `description = Optional.empty()`
  - then: 둘 다 기존 값으로 `update` 호출 → 실질적 변경 없음
- `should_WorkspaceInvalidNameException_when_name_blank`
  - given: `name = Optional.of("   ")` → `workspace.update()` throws `IllegalArgumentException`
