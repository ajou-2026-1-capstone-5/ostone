...
> **Backlog**: 로그인 한 사용자가 워크스페이스를 생성·조회·수정·삭제하고 싶다 → 모든 도메인 리소스(corpus, domain pack 등)가 워크스페이스 단위로 격리되므로, 워크스페이스 자체의 기본 CRUD가 필요
...
**201 Created** — WorkspaceResponse DTO와 동일 스키마 (myRole, updatedAt 사용)
...
> `name` 검증 실패(blank·null·255초과) → `WorkspaceInvalidNameException`(400 `WORKSPACE_INVALID_NAME`). `description` 검증 실패(2000초과) → `WorkspaceInvalidDescriptionException`(400 `WORKSPACE_INVALID_DESCRIPTION`). UseCase에서 도메인 호출 전 필드별로 개별 검증하여 구분한다.
...
**204 No Content** — 소프트 삭제 성공 (응답 바디 없음); 이미 ARCHIVED 상태인 경우에도 204 반환 (idempotent)
...
│   └── package-info.java
├── application/
│   ├── CreateWorkspaceUseCase.java        (@Service @Transactional(readOnly=true), execute()는 @Transactional 오버라이드)
...
│   ├── UpdateWorkspaceUseCase.java        (@Service @Transactional(readOnly=true), execute()는 @Transactional 오버라이드)
│   ├── ArchiveWorkspaceUseCase.java       (@Service @Transactional(readOnly=true), execute()는 @Transactional 오버라이드)
...
│   │   ├── WorkspaceInvalidDescriptionException.java      (extends BadRequestException)
...
public class UpdateWorkspaceUseCase {
  ...
  public WorkspaceResult execute(UpdateWorkspaceCommand command) {
    ...
    // 도메인 호출 전 어느 필드가 실패했는지 명확히 구분하기 위해 name/description을 개별 검증
    if (newName == null || newName.isBlank()) {
      throw new WorkspaceInvalidNameException("워크스페이스 이름이 올바르지 않습니다.");
    }
    if (newName.length() > 255) {
      throw new WorkspaceInvalidNameException("워크스페이스 이름은 255자를 초과할 수 없습니다.");
    }
    if (newDescription != null && newDescription.length() > 2000) {
...
public class ArchiveWorkspaceUseCase {
  ...
  public void execute(Long workspaceId, Long userId) {
    ...
    // idempotent: 이미 ARCHIVED 상태면 재시도로 간주하고 정상 반환 (204)
...
| `WorkspaceInvalidDescriptionException`  | `BadRequestException`        | 400  | `WORKSPACE_INVALID_DESCRIPTION` |
...
- `should_WorkspaceInvalidNameException_when_name_blank`
  - given: `name = JsonNullable.of("   ")` → UseCase explicit validation에서 즉시 예외
  - then: `WorkspaceInvalidNameException` (400) 발생
- `should_WorkspaceInvalidNameException_when_name_null_명시전달`
  - given: `name = JsonNullable.of(null)` → UseCase explicit validation에서 즉시 예외
  - then: `WorkspaceInvalidNameException` (400) 발생
- `should_WorkspaceInvalidDescriptionException_when_description_2000초과`
  - given: `name = JsonNullable.undefined()`, `description = JsonNullable.of("x".repeat(2001))`
  - then: `WorkspaceInvalidDescriptionException` (400 `WORKSPACE_INVALID_DESCRIPTION`) 발생
...
- `should_정상반환_when_이미_ARCHIVED상태`
  - given: workspace.status = ARCHIVED
  - then: 예외 없이 정상 반환, `workspace.archive()` 미호출, `workspaceRepository.save` 미호출
...
- `should_400WORKSPACE_INVALID_NAME_when_name_blank` — `{"name":"   "}` 전송 → 400 `WORKSPACE_INVALID_NAME`
- `should_400WORKSPACE_INVALID_NAME_when_name_null_전달` — `{"name":null}` 전송 → 400 `WORKSPACE_INVALID_NAME`
- `should_400WORKSPACE_INVALID_NAME_when_name_255초과` — `{"name":"x...x(256자)"}` 전송 → 400 `WORKSPACE_INVALID_NAME`
- `should_400WORKSPACE_INVALID_DESCRIPTION_when_description_2000초과` — `{"description":"x...x(2001자)"}` 전송 → 400 `WORKSPACE_INVALID_DESCRIPTION`
...
- `should_204반환_when_이미_ARCHIVED_상태` — 중복 삭제 시 idempotent 204 반환
...
- **SecurityConfig `authenticationEntryPoint` 커스터마이징**: JWT 필터 단계에서 인증 실패 시 `HttpStatusEntryPoint`는 bare 401만 반환하는데, 스펙이 요구하는 JSON 응답(`{"code":"UNAUTHORIZED","message":"..."}`)을 위해서는 `AuthenticationEntryPoint` 구현을 교체해야 한다. `GlobalExceptionHandler`가 처리하는 `AuthenticationCredentialsNotFoundException`(컨트롤러 도달 후 실패)과는 다르므로, 필터 단계 인증 실패도 JSON 형식으로 응답하도록 customEntryPoint 클래스 구현 필요.
