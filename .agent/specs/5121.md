...
> **Branch**: `fix/512-domain-pack-flow-refine`
...
대신 다음 조건을 만족하는 1개 버전을 current version으로 간주한다.
...
5121 이후 Domain Pack 목록/상세의 current version 기준은 과거의 `versionNo` 최대 PUBLISHED 기준을 대체한다. current는 PUBLISHED 중 DRAFT Intent가 없고 `publishedAt`이 가장 최신인 version이다.

### Workspace / Pack 범위

현재 FE 스펙 `512.md`에서는 workspace 기준 운영중인 도메인팩이 최대 1개로 표시되는 것을 기대한다.

따라서 목록 API에서는 workspace 기준 최신 배포 가능 PUBLISHED version 1개를 current로 간주한다. 그 version이 속한 pack만 `currentVersionId`를 가진다.

상세 API에서는 해당 pack 기준 최신 배포 가능 PUBLISHED version 1개를 current로 간주한다. 상세 화면은 특정 pack 안에서 버전 목록과 배포중 배지를 보여주기 때문이다.

| API | current 계산 범위 |
| --- | --- |
| `GET /api/v1/workspaces/{workspaceId}/domain-packs` | workspace 기준 최신 배포 가능 PUBLISHED version |
| `GET /api/v1/workspaces/{workspaceId}/domain-packs/{packId}` | pack 기준 최신 배포 가능 PUBLISHED version |
...
| GET | `/api/v1/workspaces/{workspaceId}/domain-packs/{packId}` | pack 상세와 pack 기준 current version 정보 조회 |
...
상세 API의 current 기준은 pack 기준 latest 배포 가능 PUBLISHED version이다.
...
| Domain Pack list/detail query | workspace/pack 기준 current version 필드 계산 |
...
3. pack 기준 latest 배포 가능 PUBLISHED version을 조회한다.
4. `DomainPackDetailResult`에 `currentVersion*` 필드를 포함해 반환한다.
...
- `PUBLISHED`가 정확히 1개일 때 current로 추론하는 fallback은 backend 상세 응답이 준비되면 제거한다.
...
| pack 상세에 PUBLISHED version이 없음 | 상세 `currentVersion*`가 null |
| pack 상세에 여러 배포 가능 PUBLISHED version이 있음 | pack 기준 가장 최신 배포 가능 PUBLISHED version이 current |
| pack의 PUBLISHED version에 DRAFT Intent가 있음 | 상세 `currentVersion*` 계산에서 제외 |
...
| PUBLISHED version deploy 성공 후 | 그 version이 최신 PUBLISHED가 되어 상세에서 `currentVersionId`로 반환 |
...
| Current query | workspace/pack 기준 current가 DRAFT Intent 없는 최신 PUBLISHED version으로 계산됨 |
...
- workspace 기준 current pack 1개 정책과 pack 기준 current version 1개 정책을 혼동하지 않도록 API별 기준을 문서와 테스트에 고정해야 한다.
