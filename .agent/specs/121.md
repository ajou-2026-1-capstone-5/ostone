...
sequenceDiagram
    ...
    Note over svc: 전수 선행 파싱 (DB write 없음)
    loop for each conversation
	    svc ->> parser: parse(consultingContent)
	    parser -->> svc: List<TurnData> or ConsultingContentParseException → 400
	  end
...
| `conversations` | O | min 1, max 1000 |
...
```json
{
  "error": "CONSULTING_CONTENT_PARSE_ERROR",
