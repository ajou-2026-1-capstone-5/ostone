...
sequenceDiagram
    ...
    be ->> be: .agent/specs/4.1.3-demo-runtime-domain-pack-mock.md 정의
...
| 규칙 | 내용 |
|------|------|
| `id` | DB 저장을 가정한 deterministic numeric ID. 100번대 범위 사용 (pack 100, version 101, intent 110-112, slot 120-125, policy 130-134, risk 140-142, workflow 150-152, binding 160-167) |
...
| `policies` | 5 | 취소가능확인, 주소변경제한, 환불금액확인, 고액취소알림, 반품기한확인 |
...
```json
{
  ...
  "nodes": [
    ...
    { "id": "n3", "type": "ACTION", "label": "취소 처리", "policyRef": "cancel_policy" },
...
→ ACTION 노드에서 cancel_policy policyRef로 취소 정책 적용
→ TERMINAL 상태 도달
```
...
| `cancel_policy` | `cancel_order_flow` | 취소 처리 |
...
| A10 | MUST NOT 가드레일 6개 항목이 명시됨 | MUST NOT 섹션 확인 |
