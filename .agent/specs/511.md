...
| 사이드바 | collapsed/expanded 전환 버튼과 256px expanded width | 접기 버튼 제거, 고정형 compact sidebar로 축소 | 데모 화면 가로 공간 확보 |
...
- hover preview는 가능하면 TanStack Query cache를 활용한다.
- hover가 짧게 지나가는 경우 불필요한 요청을 줄이기 위해 다음 중 하나를 선택한다.
  - `WorkflowGraphMini`가 현재처럼 실제 렌더 시 detail query를 수행한다.
  - `WorkflowCard` hover/focus에서 `queryClient.prefetchQuery`를 호출한다.
...
| sidebar collapsed state | 제거 예정 | 데모에서는 고정 compact sidebar 사용 |
...
- 기존 `열기` 버튼은 제거하거나 보조 액션으로 낮춘다. 기본 동작은 카드 전체 click 이동이다.
...
- sidebar는 데모에 필요한 nav item이 보이는 compact width를 사용한다.
- 기존 expanded `256px`, collapsed `72px` 기준에서 버튼 제거 후 적정 width를 새로 정한다. 권장 범위는 `64px-72px`이다.
...
- preview query는 렌더링 시점 또는 짧은 prefetch 기준으로 제한한다.
