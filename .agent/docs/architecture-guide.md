...
- **Spring Boot Backend**: 인증/인가(auth), 상담 로그 업로드(corpus), domain pack 버전 생성 및 활성화(domainpack), 상담 세션 관리(workflowruntime)가 구현되어 있다.
- **PostgreSQL**: Docker Compose로 PostgreSQL 17을 운영하며, app/corpus/pack/runtime 스키마의 핵심 테이블이 동작한다.
- **Frontend**: 로그인, 회원가입, 비밀번호 재설정, 상담 로그 업로드, 상담 채팅 페이지가 구현되어 있다.
- **ML Pipeline**: 6개 파이프라인 stage의 진입점(`main.py`)과 공통 유틸리티 구조가 마련되어 있다.

**목표(계획)**:

- **운영자 웹 콘솔(Frontend)**: domain pack 조회/검토/승인, 파이프라인 실행 상태 확인 화면을 제공한다. (페이지 디렉터리만 존재)
- **고객 채팅 데모 UI(Frontend)**: workflow runtime 동작을 시연하는 화면이다. (미구현)
- **Airflow + Python Pipeline**: 상담 로그 기반 domain pack 초안을 생성하는 오프라인 파이프라인이다. (Airflow 연동 미구현)
- **LLM / Inference Layer**: 요약, 정규화, 구조화, 라벨링 등에 선택적으로 활용되는 외부 추론 계층이다. (미구현)
- **review / pipeline-job / chat-demo BC**: 각 bounded context의 도메인 모델과 유스케이스가 구현될 예정이다. (패키지 스켈레톤만 존재)
